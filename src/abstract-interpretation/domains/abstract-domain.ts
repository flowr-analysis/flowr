 ...
 */
export interface AbstractDomain<Domain extends AbstractDomain<Domain, Concrete, Abstract, Top, Bot>, Concrete, Abstract, Top = typeof Top, Bot = typeof Bottom, Value extends Abstract | Top | Bot = Abstract | Top | Bot>
extends Lattice<Domain, Abstract, Top, Bot, Value> {
	/**
	 ...
	 */
	abstract(concrete: ReadonlySet<Concrete> | typeof Top): Domain;
}
...
export interface SatisfiableDomain<T> {
	...
	/**
	 * Checks whether the current abstract value is an upper bound for a concrete value (i.e. the concrete value is less or equal than the upper bound of the abstract value).
	 * @see {@link Ternary} for the returned satisfiability result
	 */
	satisfiesLeq(value: T): Ternary;
}

/**
 * A type representing any abstract domain without additional information.
 */
export type AnyAbstractDomain = AbstractDomain<AnyAbstractDomain, unknown, unknown, unknown, unknown>;
...
export type ConcreteDomain<Domain extends AnyAbstractDomain> =
	Domain extends AbstractDomain<infer _Domain, infer Concrete, unknown, unknown, unknown> ? Concrete : never;
...
export type AbstractDomainValue<Domain extends AnyAbstractDomain> =
	Domain extends AbstractDomain<infer _Domain, unknown, infer Value, infer Top, infer Bot> ? Value | Top | Bot : never;
...
export type AbstractDomainTop<Domain extends AnyAbstractDomain> =
	Domain extends AbstractDomain<infer _Domain, unknown, unknown, infer Top, unknown> ? Top : never;
...
export type AbstractDomainBottom<Domain extends AnyAbstractDomain> =
	Domain extends AbstractDomain<infer _Domain, unknown, unknown, unknown, infer Bot> ? Bot : never;
