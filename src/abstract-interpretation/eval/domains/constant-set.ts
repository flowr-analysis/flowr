	...
	infer: function(node: Node, deps: ReadonlyMap<NodeId, Lift<ConstSet>>): Lift<ConstSet> {
		switch(node.type) {
			...
			case 'concat': {
				...
				if(separator.kind !== 'const-set') {
					return Top;
				}
				if(!values.every(it => it.kind === 'const-set')) {
					return Top;
				}
			...
			case 'join': {
				...
				const values = node.params.map(nid => deps.get(nid) ?? Top);
				if(!values.every(v => v.kind === 'const-set')) {
					return Top;
				}
			...
			case 'casing': {
				...
				if(value.kind !== 'const-set') {
					return value;
				}
				let result;
				if(node.to === 'upper') {
					result = value.value.map(it => it.toUpperCase());
				} else if(node.to === 'lower') {
					result = value.value.map(it => it.toLowerCase());
				} else {
			...
			case 'function': {
				switch(node.name) {
					case 'sprintf': {
						...
						if(!positional.every(it => it.kind === 'const-set')) {
							return Top;
						}
