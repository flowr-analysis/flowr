	...
	infer: function(node: Node, deps: ReadonlyMap<NodeId, Lift<Presuffix>>): Lift<Presuffix> {
		switch(node.type) {
			...
			case 'concat': {
				...
				const nonTopPrefixes = takeWhile(values, it => it.kind === 'presuffix') as Presuffix[];
				const exactPrefixes = takeWhile(nonTopPrefixes, it => it.exact === true);
				let prefix = '';
				...
				const nonTopSuffixes = takeWhile(values.toReversed(), it => it.kind === 'presuffix') as Presuffix[];
				const exactSuffixes = takeWhile(nonTopSuffixes, it => it.exact === true);
				let suffix = '';
			...
			case 'join': {
				...
				if(!values.every(v => v.kind === 'presuffix')) {
					return Top;
				}
				const result = values.reduce((l, r) => {
					const prefix = longestCommonPrefix(l.prefix, r.prefix);
					const suffix = longestCommonSuffix(l.suffix, r.suffix);
					const exact = l.exact && r.exact && l.prefix === r.prefix;
					return { kind: 'presuffix', prefix, suffix, exact };
				});
				if(result.prefix === '' && result.suffix === '' && !result.exact) {
					return Top;
				} else {
					return result;
				}
			...
			case 'casing': {
				...
				if(value.kind !== 'presuffix') {
					return value;
				}
				const result: Presuffix = { ...value };
				if(node.to === 'upper') {
			...
			case 'function': {
				switch(node.name) {
					case 'sprintf': {
						...
						if(!positional.every(it => it.kind === 'presuffix' && it.exact)) {
							return Top;
						}
