import { NodeId as AstNodeId } from "../../r-bridge/lang-4.x/ast/model/processing/node-id";
...
export class Graph {
  ...
  insertIfMissing(nid: NodeId, node: Node): NodeId {
    if (this._nodes.has(nid)) return nid
    else return this.insertNode(nid, node)
  }
  ...
  inferValues<T extends Value>(domain: Domain<T>, max_updates: number = 100): Map<NodeId, Lift<T>> {
    ...
    while ((nid = dirty.pop()) !== undefined) {
      ...
      if (update > max_updates) continue;
