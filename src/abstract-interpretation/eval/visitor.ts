...
import type { BuiltInFunctionOrigin } from '../../dataflow/origin/dfg-get-origin';
					...
					if(sepId !== undefined) {
						...
						if(valId !== undefined) {
							...
							if(!origins || origins.length !== 1) {
								separator = this.graph.insertIfMissing(sepId, { type: 'unknown' });
							} else {
								separator = this.graph.insertNode(sepId, { type: 'alias', to: origins[0] });
							}
						} else {
