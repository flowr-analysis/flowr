 ...
 */
interface BaseSteppingSlicerInput extends MergeableRecord {
...
interface NormalizeSteppingSlicerInput extends BaseSteppingSlicerInput {
	tokenMap: TokenMap
}

interface DecorateSteppingSlicerInput extends NormalizeSteppingSlicerInput {
	getId: IdGenerator<NoInfo>
}


interface SliceSteppingSlicerInput extends DecorateSteppingSlicerInput {
	criterion: SlicingCriteria
}
 ...
 */
export type SteppingSlicerInput<InterestedIn extends SubStepName[]> =
// we are interested in a step that requires the slicing criteria
	'reconstruct'[] extends InterestedIn ? SliceSteppingSlicerInput :
		'slice'[] extends InterestedIn ? SliceSteppingSlicerInput :
			'decode criteria'[] extends InterestedIn ? SliceSteppingSlicerInput :
				// we are interested in a step that does not require the slicing criteria but at least needs to normalize
				'dataflow'[] extends InterestedIn ? DecorateSteppingSlicerInput :
					'decorate'[] extends InterestedIn ? DecorateSteppingSlicerInput :
						// we are interested in normalize step which does not yet require the id generator
						'normalize ast'[] extends InterestedIn ? NormalizeSteppingSlicerInput :
							// we are only interested in the parse step
							BaseSteppingSlicerInput
