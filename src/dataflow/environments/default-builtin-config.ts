...
const MagrittrPipePackages = [PkgName.Magrittr, PkgName.Dplyr, PkgName.Purrr, PkgName.Stringr, PkgName.Tibble,
	PkgName.TidyR, PkgName.Testthat, PkgName.Magick, PkgName.Promises] as const;
...
/**
 * Label every name the R language itself provides {@link CallProp.Primitive}, splitting an entry that mixes
 * them with names R does not (one definition covers `base::stopifnot` and `assertthat::assert_that`, and only
 * the first is R's). {@link RBasePrimitives} is read out of a real R, so this states what that R has.
 */
function markPrimitives(definitions: BuiltInDefinitions): BuiltInDefinitions {
	const out: BuiltInDefinitions = [];
	for(const def of definitions) {
		if(def.type !== 'function') {
			out.push(def);
		...
		let primitives: Identifier[] | undefined = undefined;
		let rest: Identifier[] | undefined = undefined;
		for(const name of def.names) {
			/* the set is base's own, so a name of another namespace is not the one R provides */
			if(RBasePrimitives.has(Identifier.getName(name)) && (Identifier.getNamespace(name) ?? PkgName.Base) === PkgName.Base) {
		...
		out.push({ ...def, names: primitives, config: { ...def.config, props: (def.config?.props ?? 0) | CallProp.Primitive } });
		if(rest !== undefined) {
			out.push({ ...def, names: rest });
		}
	}
...
export const WrittenBuiltinDefinitions = [
	...
	{ type: 'function', names: [Identifier.from(['~', PkgName.Base])], processor: BuiltInProcName.DefaultReadAllArgs, config: { markArgsAsMasked: NseArguments.All, sig: [['y', ArgProp.Nse], ['model', ArgProp.NoDefault | ArgProp.Nse]] }, assumePrimitive: false },
	...
	{ type: 'function', names: Identifier.fromAll(PkgName.Base, ['length', 'lengths', 'nrow', 'ncol', 'NROW', 'NCOL', 'dim', 'is.null', 'is.factor', 'is.vector', 'is.matrix', 'is.data.frame', 'is.numeric', 'is.character', 'is.logical', 'is.function', 'is.list', 'is.double', 'is.integer', 'is.complex', 'is.raw', 'is.single', 'is.ordered']), processor: BuiltInProcName.DefaultReadAllArgs, config: { props: CallProp.Pure, tags: [SemanticCallTag.Narrows], sig: SigShape }, assumePrimitive: true },
	...
	{ type: 'function', names: Identifier.fromAll(PkgName.Base, ['numeric', 'character', 'logical', 'integer', 'double', 'raw', 'single']), processor: BuiltInProcName.DefaultReadAllArgs, config: { props: CallProp.Pure, sig: [['length', ArgProp.Forced | ArgProp.Value]] }, assumePrimitive: true },
	...
	{ type: 'function', names: Identifier.fromAll(PkgName.Xml2, ['download_html', 'download_xml']), processor: BuiltInProcName.Default, config: { libFn: true, tags: [SemanticCallTag.Network, SemanticCallTag.File, SemanticCallTag.Writes], sig: [['url', ArgProp.Forced | ArgProp.Resource], ['file', ArgProp.Forced | ArgProp.Resource], ['...', ArgProp.Forced | ArgProp.Value]] }, assumePrimitive: false },
	...
	{ type: 'function', names: [Identifier.from(['gcs_get_object', PkgName.GoogleCloudStorageR])], processor: BuiltInProcName.Default, config: { libFn: true, tags: [SemanticCallTag.Network, SemanticCallTag.File, SemanticCallTag.Writes], sig: [['object_name', ArgProp.Forced | ArgProp.Resource], ['saveToDisk', ArgProp.Forced | ArgProp.Resource], ['...', ArgProp.Forced | ArgProp.Value]] }, assumePrimitive: false },
	...
	{ type: 'function', names: [Identifier.from(['jitter', PkgName.Base])], processor: BuiltInProcName.DefaultReadAllArgs, config: { tags: [SemanticCallTag.Random], sig: [['x', ArgProp.NoDefault], ['factor', 0], ['amount', 0]] }, assumePrimitive: true },
	...
	{ type: 'function', names: [...Identifier.fromAll(PkgName.Base, ['apply', 'tapply']), Identifier.from(['Tapply', PkgName.Functools])], processor: BuiltInProcName.Apply, config: { indexOfFunction: 2, nameOfFunctionArgument: 'FUN', unquoteFunction: true, props: CallProp.MayPure, sig: [['X', ArgProp.NoDefault], ['...', 0]] }, assumePrimitive: false },
	...
	{ type: 'function', names: [Identifier.from(['tryCatch', PkgName.Base]), Identifier.from(['tryCatchLog', PkgName.TryCatchLog])], processor: BuiltInProcName.Try, config: { block: 'expr', handlers: { error: 'error', finally: 'finally' }, sig: [['expr', ArgProp.Value | ArgProp.Forced], ['...', ArgProp.Callee], ['finally', ArgProp.Nse]] }, assumePrimitive: true },
	{ type: 'function', names: [Identifier.from(['withCallingHandlers', PkgName.Base])], processor: BuiltInProcName.Try, config: { block: 'expr', aborts: false, handlers: {}, sig: [['expr', ArgProp.Value | ArgProp.Forced], ['...', ArgProp.Callee]] }, assumePrimitive: true },
	...
	{ type: 'function', names: ['['], processor: BuiltInProcName.Access, config: { treatIndicesAsString: false, props: CallProp.Pure, sig: [['x', ArgProp.Forced | ArgProp.Value], ['...', ArgProp.Value]] }, assumePrimitive: true },
	{ type: 'function', names: ['[['], processor: BuiltInProcName.Access, config: { treatIndicesAsString: false, resolveField: true, props: CallProp.Pure, sig: [['x', ArgProp.Forced | ArgProp.Value], ['...', ArgProp.Value]] }, assumePrimitive: true },
	/* the field is a name rather than a value, so it is never evaluated */
	{ type: 'function', names: ['$', '@'], processor: BuiltInProcName.Access, config: { treatIndicesAsString: true, resolveField: true, props: CallProp.Pure, sig: [['x', ArgProp.Forced | ArgProp.Value], ['name', ArgProp.Nse]] }, assumePrimitive: true },
	{ type: 'function', names: ['::'], processor: BuiltInProcName.NamespaceAccess, config: { internal: false, sig: [['pkg', ArgProp.NoDefault], ['name', ArgProp.NoDefault]] }, assumePrimitive: true },
	{ type: 'function', names: [':::'], processor: BuiltInProcName.NamespaceAccess, config: { internal: true, sig: [['pkg', ArgProp.NoDefault], ['name', ArgProp.NoDefault]] }, assumePrimitive: true },
	{ type: 'function', names: ['if'], processor: BuiltInProcName.IfThenElse, config: { sig: [['cond', ArgProp.NoDefault | ArgProp.Forced | ArgProp.Atomic], ['cons.expr', ArgProp.NoDefault], ['alt.expr', 0]] }, assumePrimitive: true },
	{ type: 'function', names: [Identifier.from(['ifelse', PkgName.Base]), Identifier.from(['fifelse', PkgName.DataTable]), Identifier.from(['IfElse', PkgName.Functools])], processor: BuiltInProcName.IfThenElse, config: { args: { cond: 'test', yes: 'yes', no: 'no' }, props: CallProp.Pure, sig: [['test', ArgProp.NoDefault], ['yes', ArgProp.NoDefault], ['no', ArgProp.NoDefault]] }, assumePrimitive: true },
	...
	{ type: 'function', names: [Identifier.from(['get', PkgName.Base])], processor: BuiltInProcName.Get, evalHandler: BuiltInEvalName.Get, config: { props: CallProp.Pure, tags: [SemanticCallTag.Eval], sig: [['x', ArgProp.Forced | ArgProp.Value | ArgProp.Injectable], ['pos', ArgProp.Flag], ['envir', ArgProp.Forced | ArgProp.Value], ['mode', ArgProp.Forced | ArgProp.Flag], ['inherits', ArgProp.Forced | ArgProp.Flag]] }, assumePrimitive: false },
	{ type: 'function', names: [Identifier.from(['get0', PkgName.Base])], processor: BuiltInProcName.Get, evalHandler: BuiltInEvalName.Get, config: { props: CallProp.Pure, tags: [SemanticCallTag.Eval], sig: [['x', ArgProp.Forced | ArgProp.Value | ArgProp.Injectable], ['envir', ArgProp.Forced | ArgProp.Value], ['mode', ArgProp.Forced | ArgProp.Flag], ['inherits', ArgProp.Forced | ArgProp.Flag], ['ifnotfound', ArgProp.Forced | ArgProp.Value]] }, assumePrimitive: false },
	{ type: 'function', names: [Identifier.from(['ls', PkgName.Base]), Identifier.from(['objects', PkgName.Base])], processor: BuiltInProcName.EnvContents, config: { props: CallProp.Pure, sig: [['name', ArgProp.Forced | ArgProp.Value], ['pos', ArgProp.Flag], ['envir', ArgProp.Forced | ArgProp.Value], ['all.names', ArgProp.Flag], ['pattern', ArgProp.Value], ['sorted', ArgProp.Flag]] }, assumePrimitive: false },
	{ type: 'function', names: [Identifier.from(['list2env', PkgName.Base])], processor: BuiltInProcName.ListToEnv, config: { props: CallProp.Scope, sig: [['x', ArgProp.Forced | ArgProp.Value], ['envir', ArgProp.Written]] }, assumePrimitive: false },
	...
	{ type:      'function', names:     [Identifier.from(['setValidity', PkgName.Methods])],
		...
		config:    {
			...
			props:      CallProp.Scope | CallProp.Invisible, sig:        [['Class', ArgProp.NoDefault], ['method', ArgProp.NoDefault], ['where', 0]]
		}, assumePrimitive: true },
	{ type:      'function', names:     [Identifier.from(['setIs', PkgName.Methods])],
		...
		config:    {
			...
			props:     CallProp.Scope | CallProp.Invisible, sig:       [['class1', ArgProp.NoDefault], ['class2', ArgProp.NoDefault], ['test', 0], ['coerce', 0], ['replace', 0], ['by', 0], ['where', 0], ['classDef', 0], ['extensionObject', 0], ['doComplete', 0]]
		}, assumePrimitive: true },
	...
	{ type: 'function', names: [Identifier.from(['data', PkgName.Utils]), Identifier.from(['getHdata', PkgName.Hmisc])], processor: BuiltInProcName.DefineArgument, config: { superAssignment: true, sig: [['...', 0], ['list', 0], ['package', 0], ['lib.loc', 0], ['verbose', 0], ['envir', 0], ['overwrite', 0]] }, assumePrimitive: false },
	/* only `&&`/`||` short-circuit */
	{ type: 'function', names: [Identifier.from(['&&', PkgName.Base])], processor: BuiltInProcName.SpecialBinOp, config: { lazy: true, evalRhsWhen: true, props: CallProp.Pure, sig: [['x', ArgProp.NoDefault | ArgProp.Forced | ArgProp.Atomic], ['y', ArgProp.NoDefault | ArgProp.Atomic]] }, assumePrimitive: true, evalHandler: BuiltInEvalName.Logical },
	{ type: 'function', names: [Identifier.from(['||', PkgName.Base])], processor: BuiltInProcName.SpecialBinOp, config: { lazy: true, evalRhsWhen: false, props: CallProp.Pure, sig: [['x', ArgProp.NoDefault | ArgProp.Forced | ArgProp.Atomic], ['y', ArgProp.NoDefault | ArgProp.Atomic]] }, assumePrimitive: true, evalHandler: BuiltInEvalName.Logical },
	{ type: 'function', names: Identifier.fromAll(PkgName.Base, ['&', '|']), processor: BuiltInProcName.SpecialBinOp, config: { lazy: false, props: CallProp.Pure, sig: [['e1', ArgProp.NoDefault | ArgProp.Forced | ArgProp.Value | ArgProp.Atomic], ['e2', ArgProp.NoDefault | ArgProp.Forced | ArgProp.Value | ArgProp.Atomic]] }, assumePrimitive: true, evalHandler: BuiltInEvalName.Logical },
	...
	{ type: 'function', names: ['function', '\\'], processor: BuiltInProcName.FunctionDefinition, config: { sig: [['arglist', ArgProp.Nse], ['expr', ArgProp.NoDefault | ArgProp.Nse]] }, assumePrimitive: true },
	...
	{ type: 'function', names: [Identifier.from(['setRefClass', PkgName.Methods])], processor: BuiltInProcName.ClassGenerator, config: { classDecl: { system: ClassSystem.RefClass, nameArg: { idx: 0, name: 'Class' }, containsArg: { idx: 2, name: 'contains' }, memberArgs: [{ idx: 1, name: 'fields', typed: true }, { idx: 3, name: 'methods', methods: true }] }, sig: [['Class', ArgProp.NoDefault], ['fields', 0], ['contains', 0], ['methods', 0], ['where', 0], ['inheritPackage', 0], ['...', 0]] }, assumePrimitive: false },
	...
	{ type: 'function', names: [Identifier.from(['attach', PkgName.Base])], processor: BuiltInProcName.Attach, config: { sig: [['what', ArgProp.NoDefault], ['pos', 0], ['name', 0], ['warn.conflicts', 0]] }, assumePrimitive: false },
	{ type: 'function', names: ['for'],    processor: BuiltInProcName.ForLoop,    config: { sig: [['var', ArgProp.NoDefault | ArgProp.Nse], ['seq', ArgProp.NoDefault | ArgProp.Forced], ['expr', ArgProp.NoDefault]] }, assumePrimitive: true },
	{ type: 'function', names: ['repeat'], processor: BuiltInProcName.RepeatLoop, config: { sig: [['expr', ArgProp.NoDefault | ArgProp.Forced]] }, assumePrimitive: true },
	{ type: 'function', names: ['while'],  processor: BuiltInProcName.WhileLoop,  config: { sig: [['cond', ArgProp.NoDefault | ArgProp.Forced | ArgProp.Atomic], ['expr', ArgProp.NoDefault]] }, assumePrimitive: true },
	...
	{ type: 'function', names: [Identifier.from(['UseMethod', PkgName.Base])], processor: BuiltInProcName.S3Dispatch, config: { args: { generic: 'generic', object: 'object' }, props: CallProp.Generic, sig: [['generic', ArgProp.NoDefault], ['object', ArgProp.NoDefault]] }, assumePrimitive: true },
	{ type: 'function', names: [Identifier.from(['NextMethod', PkgName.Base])], processor: BuiltInProcName.S3Dispatch, config: { args: { generic: 'generic', object: 'object' }, inferFromClosure: true, props: CallProp.Generic, sig: [['generic', 0], ['object', 0], ['...', 0]] }, assumePrimitive: true },
	...
	{ type: 'function', names: [Identifier.from(['setGeneric', PkgName.Methods])], processor: BuiltInProcName.S7NewGeneric, config: { args: { name: 'name', dispatchArg: undefined, fun: 'fun' }, binds: true, sig: [['name', ArgProp.NoDefault], ['def', 0], ['group', 0], ['valueClass', 0], ['where', 0], ['package', 0], ['signature', 0], ['useAsDefault', 0], ['genericFunction', 0], ['simpleInheritanceOnly', 0]] }, assumePrimitive: true },
	...
	{ type: 'function', names: [Identifier.from(['new', PkgName.Methods])], processor: BuiltInProcName.S4Use, config: { classArgs: [{ idx: 0, name: 'Class' }], sig: [['Class', ArgProp.NoDefault], ['...', 0]] }, assumePrimitive: false },
	...
	{ type: 'function', names: [Identifier.from(['as', PkgName.Methods])], processor: BuiltInProcName.S4Use, config: { classArgs: [{ idx: 1, name: 'Class' }], sig: [['object', ArgProp.NoDefault], ['Class', ArgProp.NoDefault], ['strict', 0], ['ext', 0]] }, assumePrimitive: false },
	{ type: 'function', names: [Identifier.from(['is', PkgName.Methods])], processor: BuiltInProcName.S4Use, config: { classArgs: [{ idx: 1, name: 'class2' }], sig: [['object', ArgProp.NoDefault], ['class2', ArgProp.NoDefault]] }, assumePrimitive: false },
	{ type: 'function', names: [Identifier.from(['setAs', PkgName.Methods])], processor: BuiltInProcName.S4Use, config: { classArgs: [{ idx: 0, name: 'from' }], registersArg: { idx: 1, name: 'to' }, sig: [['from', ArgProp.NoDefault], ['to', ArgProp.NoDefault], ['def', ArgProp.NoDefault], ['replace', 0], ['where', 0]] }, assumePrimitive: false },
	{ type: 'function', names: Identifier.fromAll(PkgName.Methods, ['existsMethod', 'hasMethod', 'getMethod', 'selectMethod', 'removeMethod']), processor: BuiltInProcName.S4Use, config: { genericArg: { idx: 0, name: 'f' }, classArgs: [{ idx: 1, name: 'signature' }], sig: [['f', ArgProp.NoDefault], ['signature', 0], ['...', 0]] }, assumePrimitive: false },
	{ type:      'function', names:     [Identifier.from(['setClass', PkgName.Methods])],
		...
		}, sig: [['Class', ArgProp.NoDefault], ['representation', 0], ['prototype', 0], ['contains', 0], ['validity', 0], ['access', 0], ['where', 0], ['version', 0], ['sealed', 0], ['package', 0], ['S3methods', 0], ['slots', ArgProp.NoDefault]] }, assumePrimitive: true },
	{ type:      'function', names:     [Identifier.from(['setClassUnion', PkgName.Methods])],
		...
		}, sig: [['name', ArgProp.NoDefault], ['members', 0], ['where', 0]] }, assumePrimitive: true },
	...
	{ type: 'function', names: [Identifier.from(['Recall', PkgName.Base])], processor: BuiltInProcName.Recall, config: { libFn: true, sig: [['...', 0]] }, assumePrimitive: false },
	{ type: 'function', names: [Identifier.from(['sys.function', PkgName.Base])], processor: BuiltInProcName.Recall, config: { libFn: true, unknownOnNonZeroArg: true, props: CallProp.Lang, frame: ArgProp.Nse, sig: [['which', 0]] }, assumePrimitive: false },
	...
	{ type: 'function', names: [Identifier.from(['setNames', PkgName.Stats]), Identifier.from(['setNames', PkgName.FastUtils])], processor: BuiltInProcName.Default, config: { props: CallProp.Pure, sig: [['object', ArgProp.Forced | ArgProp.Value], ['nm', ArgProp.Forced | ArgProp.Value]] } },
	...
	{ type: 'function', names: [Identifier.from(['par', PkgName.Graphics]), Identifier.from(['tpar', PkgName.TinyPlot])], processor: BuiltInProcName.Default, config: { hasUnknownSideEffects: true, tags: [SemanticCallTag.Graphics], sig: [['...', 0], ['no.readonly', 0]] }, assumePrimitive: false },
	{
		...
		processor:       BuiltInProcName.Default,
		config:          { hasUnknownSideEffects: true, libFn: true, sig: [['chname', ArgProp.NoDefault], ['package', ArgProp.NoDefault], ['lib.loc', ArgProp.NoDefault], ['verbose', 0], ['file.ext', 0], ['...', 0]] },
		assumePrimitive: false
	},
	...
	{ type: 'function', names: [Identifier.from(['on.exit', PkgName.Base])], processor: BuiltInProcName.RegisterHook, config: { hook: KnownHooks.OnFnExit, args: { expr: { idx: 0, name: 'expr' }, add: { idx: 1, name: 'add', default: false }, after: { idx: 2, name: 'after', default: true } }, sig: [['expr', 0], ['add', 0], ['after', 0]] }, assumePrimitive: true },
	...
	/* language objects */
	{ type: 'function', names: Identifier.fromAll(PkgName.Base, ['enquote', 'call', 'as.call', 'as.expression', 'as.name', 'as.symbol', 'args', 'deparse', 'deparse1']), processor: BuiltInProcName.Default, config: { props: CallProp.Lang, sig: [['...', ArgProp.Forced]] }, assumePrimitive: false },
	...
	{ type: 'function', names: [...Identifier.fromAll(PkgName.Fs, ['file_temp', 'path_temp']), ...Identifier.fromAll(PkgName.Withr, ['local_tempfile', 'with_tempfile', 'local_tempdir', 'with_tempdir'])], processor: BuiltInProcName.Default, config: { libFn: true, tags: [SemanticCallTag.TempFile], sig: [['...', ArgProp.Forced]] }, assumePrimitive: false },
	...
	{ overrides: true, type: 'function', processor: BuiltInProcName.Default, config: { markArgsAsMasked: NseArguments.AllButFirst, props: CallProp.Pure, tags: [SemanticCallTag.Deprecated] }, names: [...Identifier.fromAll(PkgName.Dplyr, ['with_groups']), ...Identifier.fromAll(PkgName.TidyR, ['spread', 'separate_rows', 'gather', 'extract'])] },
	{ type: 'function', processor: BuiltInProcName.DefaultReadAllArgs, config: { tags: [SemanticCallTag.Deprecated] }, names: Identifier.fromAll(PkgName.GgPlot2, ['gg_dep', 'is.ggplot', 'guide_train', 'aes_auto', 'aes_'])  },
	...
	{ type: 'function', processor: BuiltInProcName.DefaultReadAllArgs, config: { tags: [SemanticCallTag.Deprecated] }, names: Identifier.fromAll(PkgName.Purrr, ['transpose', 'as_vector', 'map_dfr', 'flatten', 'reduce_right', 'map_raw', 'update_list', 'when', 'rdunif', 'rbernoulli', 'splice', 'rerun', 'prepend', 'at_depth', 'cross', 'list_along'])  },
	...
	{ type: 'function', processor: BuiltInProcName.DefaultReadAllArgs, config: { tags: [SemanticCallTag.Deprecated] }, names: Identifier.fromAll(PkgName.Tibble, ['repair_names', 'set_tidy_names', 'tidy_names', 'is.tibble', 'trunc_mat', 'frame_data', 'as.tibble', 'as_data_frame', 'lst_', 'data_frame_', 'tibble_', 'data_frame'])  },
...
export const DefaultBuiltinConfig = markPrimitives(markGenerics(WrittenBuiltinDefinitions));
