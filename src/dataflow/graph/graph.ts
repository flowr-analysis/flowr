...
import { Environment, type SerializedREnvironmentInformation , type IEnvironment, type REnvironmentInformation, toSerializedREnvironmentInformation, fromSerializedREnvironmentInformation } from '../environments/environment';
import type { AstIdMap, RNodeWithParent, SerializableAstIdMap } from '../../r-bridge/lang-4.x/ast/model/processing/decorate';
...
export type DataflowFunctionFlowInformationSerialized = Omit<DataflowFunctionFlowInformation, 'environment'> & {
...
export type FunctionDefTimingsByNode = Record<string, FunctionDefTiming>;
...
> {
	...
	public _timings: FunctionDefTimingsByNode = {};
	...
	public recordFunctionDefinitionTiming(nodeId: NodeId, datapoint: FunctionDefTiming): void {
		this._timings[String(nodeId)] = datapoint;
	}
	...
	public addVertex(vertex: DataflowGraphVertexArgument & Omit<Vertex, keyof DataflowGraphVertexArgument>, fallbackEnv: REnvironmentInformation, asRoot = true, overwrite = false): this {
		...
		// For lazy vertices, store the object as-is to preserve getters
		if('lazy' in vertex && vertex.lazy === true) {
			this.vertexInformation.set(vertex.id, vertex as unknown as Vertex);
		} else {
	...
	public mergeWith(otherGraph: DataflowGraph<Vertex, Edge> | undefined, mergeRootVertices = true): this {
		...
		// merge timings
		Object.assign(this._timings, otherGraph._timings);
