 ...
 *
 * TODO: add id map to more quickly access these ids in the future
 */
export function decorateWithIds<OtherInfo>(ast: RNode<Exclude<OtherInfo, Id>>, getId: IdGenerator<OtherInfo> = deterministicCountingIdGenerator<OtherInfo>()): IdRNode<OtherInfo> {
  const foldLeaf = (leaf: RSingleNode<OtherInfo>): IdRNode<OtherInfo> => ({ ...leaf, id: getId(leaf) })
  ...
  const foldIfThenElse = (ifThen: RIfThenElse<OtherInfo>, condition: IdRNode<OtherInfo>, then: IdRNode<OtherInfo>, otherwise?: IdRNode<OtherInfo>): IdRNode<OtherInfo> =>
    ({ ...ifThen, condition, then, otherwise, id: getId(ifThen) })
