...
registerQueryDocumentation('datatype', {
	type:             'active',
	shortDescription: 'Returns the inferred data types of the syntactic elements in the code.',
...
${
	await showQuery(shell, exampleCode, [{ type: 'datatype', criteria: ['3@x'] }], { showCode: false, collapseQuery: true, ctx })
}
...
refers to is resolved through the ${linkToQueryOfName('signature')} database, so a bare \`ad.test\` is typed with
