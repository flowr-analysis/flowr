import { LintingRule, LintingResult, LintingCertainty, LintingRuleInformation } from '../linter-format';
...
export class DeprecatedFunctionsResult extends LintingResult {
	public readonly function: string;
	public readonly range:    SourceRange;

	constructor(certainty: LintingCertainty, functionName: string, range: SourceRange) {
		super(certainty);
		this.function = functionName;
		this.range = range;
	}

	public printShort(): string {
		return `Function \`${this.function}\` at ${formatRange(this.range)}`;
	}

	public prettyPrint(): string {
		return `Function \`${this.function}\` called at ${formatRange(this.range)} is deprecated`;
	}
}
...
export class LinterRuleDeprecatedFunctions extends LintingRule<'deprecated-functions', DeprecatedFunctionsResult, DeprecatedFunctionsMetadata, DeprecatedFunctionsConfig> {
	public static DeprecatedLinterInfo = new LintingRuleInformation(
	...
	protected createSearch() {
		return Q.all().with(Enrichment.CallTargets);
	}

	protected processSearchResult(elements: FlowrSearchElements<ParentInformation>, config: DeprecatedFunctionsConfig) {
		...
		return {
			results: elements.getElements()
				...
				.map(element => (new DeprecatedFunctionsResult(
					LintingCertainty.Definitely,
					element.target,
					element.range
				))),
			'.meta': metadata
