...
import { ArgProp, SemanticProp } from '../../../dataflow/environments/built-in-props';
...
/**
 * The functions whose result is bounded no matter what flows in, read back from the {@link SemanticProp.Narrows}
...
export function narrowingFunctions(index: BuiltInIndex = BuiltInIndex.default()): readonly NarrowingFunction[] {
	...
	return index.with(SemanticProp.Narrows).map(call => {
