...
export const FloatingPointComparison = {
	/**
	...
	isNearlyLess(this: void, a: number, b: number, significantFigures: number | undefined): Ternary {
		...
		if(less === Ternary.Never) {
			...
			if(FloatingPointComparison.isNearlyEqual(a, b, significantFigures) !== Ternary.Never) {
				less = Ternary.Maybe;
			}
		}
		return less;
	...
	isNearlyLessOrEqual(this: void, a: number, b: number, significantFigures: number | undefined): Ternary {
		...
		if(leq === Ternary.Never) {
			// a is not less than b, so we check for equality with significance precision
			leq = FloatingPointComparison.isNearlyEqual(a, b, significantFigures);
		}
		return leq;
	}
};