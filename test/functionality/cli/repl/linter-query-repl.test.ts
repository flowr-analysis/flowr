...
describe('Linter Query REPL Completions', () => {
	...
	const all = Object.keys(LintingRules);
	assertReplCompletions(c, 'empty arguments',                  true,   [''],                                    ['rules:']);
	assertReplCompletions(c, 'partial prefix',                   false,  ['r'],                                   ['rules:']);
	assertReplCompletions(c, 'no rules',                         false,  ['rules:'],                              all);
	assertReplCompletions(c, 'partial rule',                     false,  ['rules:d'],                             all);
	assertReplCompletions(c, 'partial unique rule',              false,  ['rules:dead'],                          all);
	assertReplCompletions(c, 'multiple rules, one partial',      false,  ['rules:dead-code,file-path-val'],       all.filter(l => !(l === 'dead-code')));
	assertReplCompletions(c, 'multiple rules, no new one',       false,  ['rules:dead-code,file-path-validity'],  [',']);
	assertReplCompletions(c, 'multiple rules, starting new one', false,  ['rules:dead-code,file-path-validity,'], all.filter(l => !['dead-code','file-path-validity'].includes(l)));
	assertReplCompletions(c, 'all rules used',                   false,  [`rules:${all.join(',')}`],              [' ']);
