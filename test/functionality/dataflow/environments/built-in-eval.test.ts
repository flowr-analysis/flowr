...
const folded = [...getDefaultBuiltInDefinitions().builtInMemory].flatMap(([name, defs]) => defs.flatMap(d =>
	d.type === ReferenceType.BuiltInFunction && d.evalHandler !== undefined ?
		[{ name, handler: handlerOf.get(d.evalHandler), info: (d.config ?? {}) as BuiltInFnInfo }] : []));

describe('Built-in value folding', () => {
	test.each(Object.entries(FoldedBy))('%s folds the names it is configured for', (handler, names) => {
