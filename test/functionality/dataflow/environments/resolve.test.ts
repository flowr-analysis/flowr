...
describe('Resolve', withTreeSitter(shell => {
	describe.each(['env', 'graph'])('resolve with %s', (resolveWith) => {
		...
		function testResolve(
			...
			test(effectiveName, async() => {
				const { dataflow, normalize } = await createDataflowPipeline(shell, { request: requestFromInput(code.trim()) }, defaultConfigOptions).allRemainingSteps();

				const resolved = resolveIdToValue(slicingCriterionToId(identifier, normalize.idMap), {
		...
		// Do not resolve vector, if c is redefined
			...
			// Empty Vectors
			testResolve('Empty Vector',           '1@x', 'x <- c()',                      vector([]));
			testResolve('Empty Vector direct',    '1@c', 'c()',                           vector([]));
		});
