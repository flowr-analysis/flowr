...
describe.sequential('Dataflow Plot Dependencies', withShell(shell => {
	assertDataflow(label('Removing breaks link', ['functions-with-global-side-effects']), shell,
		...
		emptyGraph()
			...
			.call('2@rm', 'rm', [argumentInCall('4')], { onlyBuiltIn: true, origin: [BuiltInProcName.Rm], reads: [builtInId('rm')] })
			.calls('2@rm', builtInId('rm'))
			.returns('1@<-', '1@x')
			.use('2@x')
			.constant('1@2')
			.call('1@<-', '<-', [argumentInCall('1'), argumentInCall('0')], { onlyBuiltIn: true, origin: [BuiltInProcName.Assignment], reads: [builtInId('<-'), 1] })
			.calls('1@<-', builtInId('<-'))
			.reads('2@x', '1@x')
