	...
	describe('def-def same variable', () => {
		...
		assertDataflow(label('multiple occurrences of same variable', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'numbers', 'newlines']), shell,
			'x <- 1\nx <- 3\n3\nx <- 9', emptyGraph()
				...
				.call('5', '<-', [argumentInCall('3'), argumentInCall('4')], { returns: ['3'], reads: [NodeId.toBuiltIn('<-'), 4], onlyBuiltIn: true, environment: defaultEnv().defineVariable('x', '0', '2') })
				.calls('5', NodeId.toBuiltIn('<-'))
				.call('9', '<-', [argumentInCall('7'), argumentInCall('8')], { returns: ['7'], reads: [NodeId.toBuiltIn('<-'), 8], onlyBuiltIn: true, environment: defaultEnv().defineVariable('x', '3', '5') })
...
print(x)`, emptyGraph()
				...
				.argument('6', ['5', '0'])
				.call('9', '<-', [argumentInCall('7'), argumentInCall('8')], { returns: ['7'], reads: [NodeId.toBuiltIn('<-'), 8], onlyBuiltIn: true, environment: defaultEnv().defineFunction('{', '0', '6') })
				.calls('9', NodeId.toBuiltIn('<-'))
				.argument('9', ['8', '7'])
				.call('14', '<-', [argumentInCall('12'), argumentInCall('13')], { returns: ['12'], reads: [NodeId.toBuiltIn('<-'), 13], onlyBuiltIn: true, environment: defaultEnv().defineFunction('{', '0', '6').defineVariable('x', '7', '9') })
				...
				.argument('18', '16')
				.reads('18', '16')
				.call('18', 'print', [argumentInCall('16')], { returns: ['16'], reads: [NodeId.toBuiltIn('print')], environment: defaultEnv().defineFunction('{', '0', '6').defineVariable('x', '12', '14') })
				...
				.argument('23', '21')
				.reads('23', '21')
				.call('23', 'print', [argumentInCall('21')], { returns: ['21'], reads: [NodeId.toBuiltIn('print')], environment: defaultEnv().defineFunction('{', '0', '6').defineVariable('x', '12', '14') })
