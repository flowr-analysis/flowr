				...
				describe('With <-', () => {
					...
					assertDataflow(label('For', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'return-value-of-assignments', 'for-loop', 'numbers', 'built-in-sequencing']),
						shell, 'x <- for (i in 1:4) 3',  emptyGraph()
							...
							.call('7', 'for', [argumentInCall('1'), argumentInCall('4'), argumentInCall('5', { controlDependency: ['7'] })], { returns: [], reads: [BuiltIn, '1', '4'], onlyBuiltIn: true, environment: defaultEnv().defineVariable('i', '1', '7') })
							.nse('7', '5')
		...
		describe('For', () => {
			assertDataflow(label('simple constant for-loop', ['for-loop', 'numbers', 'name-normal', 'built-in-sequencing', 'grouping']),
				shell, 'for(i in 1:10) { 1 }', emptyGraph()
					...
					.call('7', '{', [argumentInCall('6', { controlDependency: [] })], { returns: ['6'], reads: [BuiltIn], controlDependency: [] })
					.call('8', 'for', [argumentInCall('0'), argumentInCall('3'), argumentInCall('7', { controlDependency: [] })], { returns: [], reads: ['0', '3', BuiltIn], onlyBuiltIn: true, environment: defaultEnv().defineVariable('i', '0', '8') })
					.nse('8', '7')
