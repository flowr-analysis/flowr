...
describe('Lists with if-then constructs', withShell(shell => {
			...
			describe('reads within if', () => {
					...
					describe(`${b.label}`, () => {
						...
						if(b.text !== '') {
							baseGraph
								...
								.call('12', [argumentInCall('3'), argumentInCall('7'), argumentInCall('11')], { returns: ['7', '11'], reads: ['3', BuiltIn], onlyBuiltIn: true, environment: defaultEnv().defineVariable('x', '0', '2') })
								.constant('1')
								.defineVariable('0', { definedBy: ['1', '2'] })
								.constant('6', { controlDependency: ['12'] })
								.constant('10', { controlDependency: ['12'] })
						} else {
							baseGraph.call('8', [argumentInCall('3'), argumentInCall('7'), EmptyArgument], { returns: ['7'], reads: ['3', BuiltIn], onlyBuiltIn: true, environment: defaultEnv().defineVariable('x', '0', '2') })
								.constant('1')
								.defineVariable('0', { definedBy: ['1', '2'] })
						...
						const previousGraph = emptyGraph()
							...
							.call(cd[0], [argumentInCall('3'), argumentInCall('7'), EmptyArgument], { returns: ['7'], reads: ['3', BuiltIn], onlyBuiltIn: true, environment: defaultEnv().defineVariable('x', '0', '2') })
							.constant('1')
							.defineVariable('0', { definedBy: ['1', '2'] })
							.constant('3')
				...
				assertDataflow(label('read previous def in else', [...OperatorDatabase[assign].capabilities, 'name-normal', 'numbers', 'newlines', 'if', 'logical']),
					...
					`x ${assign} 2\nif(FALSE) { 42 } else { x }`,  emptyGraph()
						...
						.call('12', [argumentInCall('3'), EmptyArgument, argumentInCall('11')], { returns: ['11'], reads: ['3', BuiltIn], onlyBuiltIn: true, environment: defaultEnv().defineVariable('x', '0', '2') })
						.constant('1')
						.defineVariable('0', { definedBy: ['1', '2'] })
						.constant('3')
				)
			...
			describe('write within if', () => {
				assertDataflow(label('without else directly together', ['if', 'logical', 'name-normal', ...OperatorDatabase[assign].capabilities, 'numbers', 'newlines']),
					...
					`if(TRUE) { x ${assign} 2 }\nx`, emptyGraph()
						...
						.call('7', [argumentInCall('0'), argumentInCall('6'), EmptyArgument], { returns: ['6'], reads: ['0', BuiltIn], onlyBuiltIn: true })
						.constant('0')
						.constant('4')
				...
				assertDataflow(label('def in else read afterwards', ['if', 'logical', 'numbers', 'name-normal', ...OperatorDatabase[assign].capabilities, 'newlines']),
					...
					`if(FALSE) { 42 } else { x ${assign} 5 }\nx`,  emptyGraph()
						.use('12')
						.reads('12', '7')
						.call('9', [argumentInCall('7'), argumentInCall('8')], { returns: ['7'], reads: [BuiltIn] })
						.call('10', [argumentInCall('9')], { returns: ['9'], reads: [BuiltIn] })
						.call('11', [argumentInCall('0'), EmptyArgument, argumentInCall('10')], { returns: ['10'], reads: ['0', BuiltIn], onlyBuiltIn: true })
						.constant('0')
						.constant('8')
						.defineVariable('7', { definedBy: ['8', '9'] })
				)
				...
				assertDataflow(label('def in then and else read afterward', ['if', 'name-normal', ...OperatorDatabase[assign].capabilities, 'numbers', 'newlines']),
					...
					`if(z) { x ${assign} 7 } else { x ${assign} 5 }\nx`,  emptyGraph()
						...
						.use('14')
						.reads('14', ['3', '9'])
						.call('5', [argumentInCall('3'), argumentInCall('4')], { returns: ['3'], reads: [BuiltIn], controlDependency: ['13'] })
						.call('6', [argumentInCall('5')], { returns: ['5'], reads: [BuiltIn], controlDependency: ['13'] })
						.call('11', [argumentInCall('9'), argumentInCall('10')], { returns: ['9'], reads: [BuiltIn], controlDependency: ['13'] })
						.call('12', [argumentInCall('11')], { returns: ['11'], reads: [BuiltIn], controlDependency: ['13'] })
						.call('13', [argumentInCall('0'), argumentInCall('6'), argumentInCall('12')], { returns: ['6', '12'], reads: ['0', BuiltIn], onlyBuiltIn: true })
						.constant('4')
						.defineVariable('3', { definedBy: ['4', '5'], controlDependency: ['13'] })
						.constant('10')
						.defineVariable('9', { definedBy: ['10', '11'], controlDependency: ['13'] })
				)
			})
		})
	}
	describe('Branch Coverage', () => {
		...
		assertDataflow(label('assignment both branches in if', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'numbers', 'newlines', 'if']), shell, 'x <- 1\nif(r) { x <- 2 } else { x <- 3 }\n y <- x',  emptyGraph()
			...
			.call('16', [argumentInCall('3'), argumentInCall('9'), argumentInCall('15')], { returns: ['9', '15'], reads: ['3', BuiltIn], onlyBuiltIn: true, environment: defaultEnv().defineVariable('x', '0', '2') })
			...
			.defineVariable('0', { definedBy: ['1', '2'] })
			.constant('7')
			.defineVariable('6', { definedBy: ['7', '8'], controlDependency: ['16'] })
			.constant('13')
			.defineVariable('12', { definedBy: ['13', '14'], controlDependency: ['16'] })
			.defineVariable('17', { definedBy: ['18', '19'] })
		)
		...
		assertDataflow(label('assignment if one branch', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'newlines', 'if', 'numbers']), shell, 'x <- 1\nif(r) { x <- 2 } \n y <- x',  emptyGraph()
			...
			.call('10', [argumentInCall('3'), argumentInCall('9'), EmptyArgument], { returns: ['9'], reads: ['3', BuiltIn], onlyBuiltIn: true, environment: defaultEnv().defineVariable('x', '0', '2', ['10']) })
			...
			.defineVariable('11', { definedBy: ['12', '13'] })
		)
		...
		assertDataflow(label('assignment if multiple variables with else', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'numbers', 'newlines', 'if']),
			...
			emptyGraph()
				...
				.call('22', [argumentInCall('6'), argumentInCall('15'), argumentInCall('21')], { returns: ['15', '21'], reads: ['6', BuiltIn], onlyBuiltIn: true, environment: defaultEnv().defineVariable('x', '0', '2').defineVariable('y', '3', '5') })
				...
				.defineVariable('3', { definedBy: ['4', '5'] })
				.constant('10')
				.defineVariable('9', { definedBy: ['10', '11'], controlDependency: ['22'] })
				.constant('13')
				.defineVariable('12', { definedBy: ['13', '14'], controlDependency: ['22'] })
				.constant('19')
				.defineVariable('18', { definedBy: ['19', '20'], controlDependency: ['22'] })
				.defineVariable('23', { definedBy: ['24', '25'] })
				.defineVariable('26', { definedBy: ['27', '28'] })
		)
		assertDataflow(label('assignment in else block', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'numbers', 'newlines', 'if']), shell, 'x <- 1 \n if(r){} else{x <- 2} \n y <- x',  emptyGraph()
			...
			.call('13', [argumentInCall('3'), argumentInCall('6'), argumentInCall('12')], { returns: ['6', '12'], reads: ['3', BuiltIn], onlyBuiltIn: true, environment: defaultEnv().defineVariable('x', '0', '2', ['13']).defineVariable('x', '9', '11', ['13']) })
			...
			.constant('10')
			.defineVariable('9', { definedBy: ['10', '11'], controlDependency: ['13'] })
			.defineVariable('14', { definedBy: ['15', '16'] })
		)
	})
}))
