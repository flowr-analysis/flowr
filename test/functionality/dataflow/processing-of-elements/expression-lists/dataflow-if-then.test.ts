...
describe.sequential('Lists with if-then constructs', withShell(shell => {
				...
				for(const b of [
					...
					describe(`${b.label}`, () => {
						...
						const baseGraph = emptyGraph()
							.use('3', 'x')
							.reads('3', '0')
							.call('2', assign, [argumentInCall('0'), argumentInCall('1')], { returns: ['0'], reads: [builtInId(assign)] })
			...
			describe('write within if', () => {
				assertDataflow(label('without else directly together', ['if', 'logical', 'name-normal', ...OperatorDatabase[assign].capabilities, 'numbers', 'newlines']),
					...
					`if(TRUE) { x ${assign} 2 }\nx`, emptyGraph()
						.use('8', 'x')
						.reads('8', '3')
						.call('5', assign, [argumentInCall('3'), argumentInCall('4')], { returns: ['3'], reads: [builtInId(assign)] })
				...
				assertDataflow(label('def in else read afterwards', ['if', 'logical', 'numbers', 'name-normal', ...OperatorDatabase[assign].capabilities, 'newlines']),
					...
					`if(FALSE) { 42 } else { x ${assign} 5 }\nx`,  emptyGraph()
						.use('12', 'x')
						.reads('12', '7')
						.call('9', assign, [argumentInCall('7'), argumentInCall('8')], { returns: ['7'], reads: [builtInId(assign)] })
				...
				assertDataflow(label('def in then and else read afterward', ['if', 'name-normal', ...OperatorDatabase[assign].capabilities, 'numbers', 'newlines']),
					...
					`if(z) { x ${assign} 7 } else { x ${assign} 5 }\nx`,  emptyGraph()
						...
						.use('14', 'x')
						.reads('14', ['3', '9'])
						.call('5', assign, [argumentInCall('3'), argumentInCall('4')], { returns: ['3'], reads: [builtInId(assign)], controlDependencies: [{ id: '13', when: true }] })
						.call('6', '{', [argumentInCall('5')], { returns: ['5'], reads: [builtInId('{')], controlDependencies: [{ id: '13', when: true }] })
						.call('11', assign, [argumentInCall('9'), argumentInCall('10')], { returns: ['9'], reads: [builtInId(assign)], controlDependencies: [{ id: '13', when: false }] })
	...
	describe('Branch Coverage', () => {
		...
		assertDataflow(label('assignment both branches in if', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'numbers', 'newlines', 'if']), shell, 'x <- 1\nif(r) { x <- 2 } else { x <- 3 }\n y <- x',  emptyGraph()
			...
			.call('8', '<-', [argumentInCall('6'), argumentInCall('7')], { returns: ['6'], reads: [builtInId('<-')], controlDependencies: [{ id: '16', when: true }], environment: defaultEnv().defineVariable('x', '0', '2') })
			...
			.call('14', '<-', [argumentInCall('12'), argumentInCall('13')], { returns: ['12'], reads: [builtInId('<-')], controlDependencies: [{ id: '16', when: false }], environment: defaultEnv().defineVariable('x', '0', '2') })
			...
			.call('19', '<-', [argumentInCall('17'), argumentInCall('18')], { returns: ['17'], reads: [builtInId('<-')], environment: defaultEnv().defineVariable('x', '6', '8', [{ id: '16', when: false }]).defineVariable('x', '12', '14', [{ id: '16', when: false }]) })
			.constant('1')
			.defineVariable('0', 'x', { definedBy: ['1', '2'] })
			.constant('7')
		...
		assertDataflow(label('assignment if one branch', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'newlines', 'if', 'numbers']), shell, 'x <- 1\nif(r) { x <- 2 } \n y <- x',  emptyGraph()
			...
			.reads('12', ['6', '0'])
			.call('2', '<-', [argumentInCall('0'), argumentInCall('1')], { returns: ['0'], reads: [builtInId('<-')] })
			.call('8', '<-', [argumentInCall('6'), argumentInCall('7')], { returns: ['6'], reads: [builtInId('<-')], controlDependencies: [{ id: '10', when: true }], environment: defaultEnv().defineVariable('x', '0', '2') })
			...
			.call('13', '<-', [argumentInCall('11'), argumentInCall('12')], { returns: ['11'], reads: [builtInId('<-')], environment: defaultEnv().defineVariable('x', '6', '8', [{ id: '10', when: true }]).defineVariable('x', '0', '2', [{ id: '10', when: true }]) })
			.constant('1')
			.defineVariable('0', 'x', { definedBy: ['1', '2'] })
			.constant('7')
		...
		assertDataflow(label('assignment if multiple variables with else', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'numbers', 'newlines', 'if']),
			...
			emptyGraph()
				...
				.call('11', '<-', [argumentInCall('9'), argumentInCall('10')], { returns: ['9'], reads: [builtInId('<-')], controlDependencies: [{ id: '22', when: true }], environment: defaultEnv().defineVariable('x', '0', '2').defineVariable('y', '3', '5') })
				.call('14', '<-', [argumentInCall('12'), argumentInCall('13')], { returns: ['12'], reads: [builtInId('<-')], controlDependencies: [{ id: '22', when: true }], environment: defaultEnv().defineVariable('x', '9', '11').defineVariable('y', '3', '5') })
				...
				.call('20', '<-', [argumentInCall('18'), argumentInCall('19')], { returns: ['18'], reads: [builtInId('<-')], controlDependencies: [{ id: '22', when: false }], environment: defaultEnv().defineVariable('x', '0', '2').defineVariable('y', '3', '5') })
				...
				.call('25', '<-', [argumentInCall('23'), argumentInCall('24')], { returns: ['23'], reads: [builtInId('<-')], environment: defaultEnv().defineVariable('x', '9', '11', [{ id: '22', when: false }]).defineVariable('x', '18', '20', [{ id: '22', when: false }]).defineVariable('y', '12', '14', [{ id: '22', when: true }]).defineVariable('y', '3', '5', [{ id: '22', when: true }]) })
				.call('28', '<-', [argumentInCall('26'), argumentInCall('27')], { returns: ['26'], reads: [builtInId('<-')], environment: defaultEnv().defineVariable('x', '9', '11', [{ id: '22', when: false }]).defineVariable('x', '18', '20', [{ id: '22', when: false }]).defineVariable('y', '12', '14', [{ id: '22', when: true }]).defineVariable('y', '3', '5', [{ id: '22', when: true }]).defineVariable('w', '23', '25') })
				.constant('1')
				.defineVariable('0', 'x', { definedBy: ['1', '2'] })
				.constant('4')
		...
		assertDataflow(label('assignment in else block', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'numbers', 'newlines', 'if']), shell, 'x <- 1 \n if(r){} else{x <- 2} \n y <- x',  emptyGraph()
			...
			.call('11', '<-', [argumentInCall('9'), argumentInCall('10')], { returns: ['9'], reads: [builtInId('<-')], controlDependencies: [{ id: '13', when: false }], environment: defaultEnv().defineVariable('x', '0', '2') })
			...
			.call('16', '<-', [argumentInCall('14'), argumentInCall('15')], { returns: ['14'], reads: [builtInId('<-')], environment: defaultEnv().defineVariable('x', '0', '2', [{ id: '13', when: false }]).defineVariable('x', '9', '11', [{ id: '13', when: false }]) })
			.constant('1')
			.defineVariable('0', 'x', { definedBy: ['1', '2'] })
			.constant('10')
			.defineVariable('9', 'x', { definedBy: ['10', '11'], controlDependencies: [{ id: '13', when: false }] })
...
print(x)`,  emptyGraph()
				...
				.call('2', '<-', [argumentInCall('0'), argumentInCall('1')], { returns: ['0'], reads: [builtInId('<-')] })
				.argument('2', ['1', '0'])
				.call('11', '<-', [argumentInCall('9'), argumentInCall('10')], { returns: ['9'], reads: [builtInId('<-')], controlDependencies: [{ id: '19', when: true }, { id: '27', when: true }], environment: defaultEnv().defineVariable('x', '0', '2') })
				.argument('11', ['10', '9'])
				.argument('12', '11')
				...
				.call('17', '<-', [argumentInCall('15'), argumentInCall('16')], { returns: ['15'], reads: [builtInId('<-')], controlDependencies: [{ id: '19', when: false }, { id: '27', when: true }], environment: defaultEnv().defineVariable('x', '0', '2') })
				.argument('17', ['16', '15'])
				.argument('18', '17')
				...
				.call('25','<-', [argumentInCall('23'), argumentInCall('24')], { returns: ['23'], reads: [builtInId('<-')], controlDependencies: [{ id: '27', when: false }], environment: defaultEnv().defineVariable('x', '0', '2') })
				.argument('25', ['24', '23'])
				.argument('26', '25')
...
a()`,  emptyGraph()
				...
				.argument('4', ['3', '0'])
				.call('15', 'assign', [argumentInCall('9'), argumentInCall('13')], { returns: ['9'], reads: [builtInId('assign')], environment: defaultEnv().defineFunction('a', '0', '4'), onlyBuiltIn: true, controlDependencies: [{ id: '17', when: true }] })
				.argument('15', ['13', '9'])
		...
		assertDataflow(label('assign from get', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'newlines', 'assignment-functions', 'strings', 'unnamed-arguments', 'name-created']),
			shell, 'b <- 5\nassign("a", get("b"))\nprint(a)', emptyGraph()
				...
				.argument('11', '9')
				.call('11', 'assign', [argumentInCall('4'), argumentInCall('9')], { returns: ['4'], reads: [builtInId('assign')], environment: defaultEnv().defineVariable('b', '0', '2') })
				.argument('11', '4')
				.argument('15', '13')
				.reads('15', '13')
