...
describe('Function Definition', withShell(shell => {
	describe('Only functions', () => {
		...
		assertDataflow(label('read of parameter in return', ['formals-named', 'return', 'name-normal']), shell, 'function(x) { return(x) }',  emptyGraph()
			.use('5', undefined, false)
			.reads('5', '0')
			.argument('7', '5')
			.call('7', [argumentInCall('5')], { returns: ['5'], reads: [BuiltIn], onlyBuiltIn: true, environment: defaultEnv().pushEnv().defineParameter('x', '0', '1') }, false)
			.argument('8', '7')
		...
		describe('x', () => {
			assertDataflow(label('return parameter named', ['formals-named', 'return', 'named-arguments']), shell, 'function(x) { return(x=x) }',  emptyGraph()
				.use('6', undefined, false)
				.reads('6', '0')
				.use('7', undefined, false)
				.reads('7', '6')
				.call('8', [argumentInCall('7', { name: 'x' } )], { returns: ['7'], reads: [BuiltIn], environment: defaultEnv().pushEnv().defineParameter('x', '0', '1') }, false)
				.argument('8', '7')
	...
	describe('Scoping of body', () => {
		...
		assertDataflow(label('local define with <- in function, read after', ['normal-definition', 'semicolons', 'name-normal', ...OperatorDatabase['<-'].capabilities, 'numbers']), shell, 'function() { x <- 3; }; x', emptyGraph()
			...
			.defineVariable('2', { definedBy: ['3', '4'] }, false)
			.defineFunction('6', ['5'], {
				out:               [],
				in:                [],
				unknownReferences: [],
		...
		assertDataflow(label('local define with -> in function, read after', ['normal-definition', 'numbers', ...OperatorDatabase['->'].capabilities, 'semicolons', 'name-normal']), shell, 'function() { 3 -> x; }; x',  emptyGraph()
			...
			.defineVariable('3', { definedBy: ['2', '4'] }, false)
			.defineFunction('6', ['5'], {
				out:               [],
				in:                [],
				unknownReferences: [],
		...
		assertDataflow(label('shadow in body',  ['name-normal', ...OperatorDatabase['<-'].capabilities, 'numbers', 'semicolons', 'normal-definition']), shell, 'x <- 2; function() { x <- 3; x }; x',  emptyGraph()
			...
			.defineVariable('5', { definedBy: ['6', '7'] }, false)
			.defineFunction('10', ['9'], {
				out:               [],
				in:                [],
				unknownReferences: [],
	...
	describe('Scoping of parameters', () => {
		assertDataflow(label('parameter shadows', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'numbers', 'semicolons', 'formals-named', 'implicit-return']), shell, 'x <- 3; function(x) { x }',  emptyGraph()
			...
			.constant('1')
			.defineVariable('0', { definedBy: ['1', '2'] })
			.defineVariable('3', { definedBy: [] }, false)
			.defineFunction('9', ['8'], {
				out:               [],
				in:                [],
				unknownReferences: [],
	...
	describe('Using named arguments', () => {
		assertDataflow(label('Read first parameter', ['formals-default', 'implicit-return', 'name-normal']), shell, 'function(a=3, b=a) { b }',  emptyGraph()
			...
			.defineVariable('0', { definedBy: ['1'] }, false)
			.constant('1', undefined, false)
			.defineVariable('3', { definedBy: ['4'] }, false)
			.defineFunction('10', ['9'], {
				out:               [],
				in:                [],
				unknownReferences: [],
		...
		assertDataflow(label('Read later definition', ['formals-named', 'name-normal', 'name-normal', 'numbers', ...OperatorDatabase['<-'].capabilities, 'semicolons', 'binary-operator', 'infix-calls', ...OperatorDatabase['+'].capabilities]), shell, 'function(a=b, m=3) { b <- 1; a; b <- 5; a + 1 }', emptyGraph()
			.use('1', undefined, false)
			.reads('1', '8')
			.use('11', undefined, false)
			.reads('11', '0')
			.use('15', undefined, false)
			.reads('15', '0')
			...
			.defineVariable('8', { definedBy: ['9', '10'] }, false)
			.constant('13', undefined, false)
			.defineVariable('12', { definedBy: ['13', '14'] }, false)
	...
	describe('Using special argument', () => {
		assertDataflow(label('Return ...', ['formals-named', 'formals-dot-dot-dot', 'unnamed-arguments', 'implicit-return']), shell, 'function(a, ...) { foo(...) }',  emptyGraph()
			...
			.reads('7', '2')
			.argument('9', '7')
			.call('9', [argumentInCall('7')], { returns: [], reads: [], environment: defaultEnv().pushEnv().defineParameter('a', '0', '1').defineParameter('...', '2', '3') }, false)
  ...
  g
			...
			.use('5', undefined, false)
			.reads('5', ['9', '19'])
			.use('12', undefined, false)
			.use('14', undefined, false)
			.reads('14', '2')
			.use('22', { controlDependencies: [] }, false)
			.reads('22', '2')
			...
			.call('16', [argumentInCall('14')], { returns: ['14'], reads: [BuiltIn], controlDependency: ['18'], environment: defaultEnv().pushEnv().defineFunction('g', '2', '8').defineVariable('y', '9', '11') }, false)
			.call('18', [argumentInCall('12'), argumentInCall('16'), EmptyArgument], { returns: ['16'], reads: ['12', BuiltIn], onlyBuiltIn: true, environment: defaultEnv().pushEnv().defineFunction('g', '2', '8').defineVariable('y', '9', '11') }, false)
			...
			.defineVariable('2', { definedBy: ['7', '8'] }, false)
			.constant('10', undefined, false)
			.defineVariable('9', { definedBy: ['10', '11'] }, false)
			.constant('20', undefined, false)
			.defineVariable('19', { definedBy: ['20', '21'], controlDependency: [] }, false)
	...
	describe('Late binding of environment variables', () => {
		assertDataflow(label('define after function definition', ['normal-definition', 'implicit-return', 'semicolons', 'name-normal', ...OperatorDatabase['<-'].capabilities, 'numbers']), shell, 'function() { x }; x <- 3',  emptyGraph()
			...
			.constant('6')
			.defineVariable('5', { definedBy: ['6', '7'] })
		)
	})
	...
	describe('Nested Function Definitions', () => {
		assertDataflow(label('double nested functions', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'normal-definition', 'unnamed-arguments', 'semicolons']), shell, 'a <- function() { x <- function(x) { x <- b }; x }; b <- 3; a',  emptyGraph()
			...
			.defineVariable('4', { definedBy: [] }, false)
			.defineVariable('8', { definedBy: ['9', '10'] }, false)
			...
			.defineVariable('3', { definedBy: ['12', '13'] }, false)
			...
			.defineVariable('0', { definedBy: ['16', '17'] })
			.constant('19')
			.defineVariable('18', { definedBy: ['19', '20'] })
		)
	})
}))
