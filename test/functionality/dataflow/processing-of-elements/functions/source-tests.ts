...
describe('source', withShell(shell => {
	...
	assertDataflow('simple source', shell, 'source("simple")\ncat(N)', emptyGraph()
		...
		.use('6', `${UnnamedArgumentPrefix}6`, {environment: envWithSimpleN})
		.argument('3', '2')
		.reads('3', BuiltIn)
	...
	assertDataflow('multiple source', shell, 'source("simple")\nN <- 0\nsource("simple")\ncat(N)', emptyGraph()
		...
		.use('2', `${UnnamedArgumentPrefix}2`)
		.use('9', `${UnnamedArgumentPrefix}9`, {environment: define({nodeId: '4', scope: 'local', name: 'N', used: 'always', kind: 'variable', definedAt: '6' }, LocalScope, initializeCleanEnvironments())})
		.use('13', `${UnnamedArgumentPrefix}13`, {environment: define({nodeId: 'simple-3:1-3:6-0', scope: 'local', name: 'N', used: 'always', kind: 'variable', definedAt: 'simple-3:1-3:6-2' }, LocalScope, initializeCleanEnvironments())})
	...
	assertDataflow('conditional', shell, 'if (x) { source("simple") }\ncat(N)', emptyGraph()
		...
		.use('3', `${UnnamedArgumentPrefix}3`)
		.use('9', `${UnnamedArgumentPrefix}9`, {environment: envWithConditionalN})
		.argument('4', '3')
	...
	assertDataflow('recursive source', shell, sources.recursive1, emptyGraph()
		...
		.use(recursive2Id(6), `${UnnamedArgumentPrefix}${recursive2Id(6)}`, {environment: envWithX})
		.use(recursive2Id(2), `${UnnamedArgumentPrefix}${recursive2Id(2)}`, {environment: envWithX})
