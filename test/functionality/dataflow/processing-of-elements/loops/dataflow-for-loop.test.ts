...
x`, emptyGraph()
				...
				.use('14', 'x')
				.reads('14', ['2', '9'])
				.call('4', '<-', [argumentInCall('2'), argumentInCall('3')], { origin: ['builtin:assignment'], returns: ['2'], reads: [builtInId('<-'), 3], onlyBuiltIn: true, cds: [{ id: '13' }, { id: '8', when: true }] })
				.calls('4', builtInId('<-'))
				.call('6', 'break', [], { origin: ['builtin:break'], returns: [], reads: [builtInId('break')], cds: [{ id: 13 }, { id: 8, when: true }], environment: defaultEnv().defineVariable('x', '2', '4') })
				.calls('6', builtInId('break'))
				.call('8', 'if', [argumentInCall('5'), argumentInCall('6'), EmptyArgument], { origin: ['builtin:if-then-else'], returns: ['6'], reads: [builtInId('if'), '5'], onlyBuiltIn: true, environment: defaultEnv().defineVariable('x', '2', '4'), cds: [{ id: '13' }, { id: '8', when: true }]  })
				.calls('8', builtInId('if'))
				.call('11', '<-', [argumentInCall('9'), argumentInCall('10')], { origin: ['builtin:assignment'], returns: ['9'], reads: [builtInId('<-'), 10], onlyBuiltIn: true, cds: [{ id: '13' }, { id: 8, when: true }], environment: defaultEnv().defineVariable('x', '2', '4') })
				.calls('11', builtInId('<-'))
				.call('12', '{', [argumentInCall('4'), argumentInCall('8'), argumentInCall('11')], { origin: ['builtin:expression-list'], returns: ['11'], reads: [builtInId('{')], environment: defaultEnv().defineVariable('x', '2', '4').defineVariable('x', '9', '11', []), cds: [{ id: '13' }, { id: '8', when: true }]  })
				.calls('12', builtInId('{'))
	...
	assertDataflow(label('Read after for loop', ['for-loop', 'name-normal', ...OperatorDatabase['<-'].capabilities, 'numbers', 'newlines']), shell, 'for(i in 1:10) { x <- 12 }\n x', emptyGraph()
		...
		.calls('3', builtInId(':'))
		.call('8', '<-', [argumentInCall('6'), argumentInCall('7')], { origin: ['builtin:assignment'], returns: ['6'], reads: [builtInId('<-'), 7], onlyBuiltIn: true, cds: [{ id: '10', when: true }] })
		.calls('8', builtInId('<-'))
		.call('9', '{', [argumentInCall('8')], { origin: ['builtin:expression-list'], returns: ['8'], reads: [builtInId('{')], cds: [{ id: '10', when: true }] })
		.calls('9', builtInId('{'))
	...
	assertDataflow(label('Read after for loop with outer def', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'numbers', 'newlines', 'for-loop']), shell, 'x <- 9\nfor(i in 1:10) { x <- 12 }\n x',  emptyGraph()
		...
		.call('6', ':', [argumentInCall('4'), argumentInCall('5')], { origin: ['builtin:default'], returns: [], reads: ['4', '5', builtInId(':')], onlyBuiltIn: true, environment: defaultEnv().defineVariable('x', '0', '2') })
		.calls('6', builtInId(':'))
		.call('11', '<-', [argumentInCall('9'), argumentInCall('10')], { origin: ['builtin:assignment'], returns: ['9'], reads: [builtInId('<-'), 10], onlyBuiltIn: true, cds: [{ id: '13', when: true }] })
		.calls('11', builtInId('<-'))
		.call('12', '{', [argumentInCall('11')], { origin: ['builtin:expression-list'], returns: ['11'], reads: [builtInId('{')], cds: [{ id: '13', when: true }] })
		.calls('12', builtInId('{'))
	...
	assertDataflow(label('double redefinition within loop', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'numbers', 'newlines', 'for-loop', 'semicolons']), shell, 'x <- 9\nfor(i in 1:10) { x <- x; x <- x }\n x', emptyGraph()
		...
		.call('6', ':', [argumentInCall('4'), argumentInCall('5')], { origin: ['builtin:default'], returns: [], reads: ['4', '5', builtInId(':')], onlyBuiltIn: true, environment: defaultEnv().defineVariable('x', '0', '2') })
		.calls('6', builtInId(':'))
		.call('11', '<-', [argumentInCall('9'), argumentInCall('10')], { origin: ['builtin:assignment'], returns: ['9'], reads: [builtInId('<-'), 10], onlyBuiltIn: true, cds: [{ id: '16', when: true }] })
		.calls('11', builtInId('<-'))
		.call('14', '<-', [argumentInCall('12'), argumentInCall('13')], { origin: ['builtin:assignment'], returns: ['12'], reads: [builtInId('<-'), 13], onlyBuiltIn: true, cds: [{ id: '16', when: true }], environment: defaultEnv().defineVariable('x', '9', '11', [{ id: '16', when: true }]) })
		.calls('14', builtInId('<-'))
		.call('15', '{', [argumentInCall('11'), argumentInCall('14')], { origin: ['builtin:expression-list'], returns: ['14'], reads: [builtInId('{')], cds: [{ id: '16', when: true }], environment: defaultEnv().defineVariable('x', '9', '11', [{ id: '16', when: true }]) })
		.calls('15', builtInId('{'))
	...
	assertDataflow(label('loop-variable redefined within loop', ['name-normal', 'for-loop', 'semicolons', 'newlines', 'numbers']), shell, 'for(i in 1:10) { i; i <- 12 }\n i', emptyGraph()
		...
		.call('3', ':', [argumentInCall('1'), argumentInCall('2')], { origin: ['builtin:default'], returns: [], reads: ['1', '2', builtInId(':')], onlyBuiltIn: true })
		.calls('3', builtInId(':'))
		.call('9', '<-', [argumentInCall('7'), argumentInCall('8')], { origin: ['builtin:assignment'], returns: ['7'], reads: [builtInId('<-'), 8], onlyBuiltIn: true, cds: [{ id: '11', when: true }] })
		.calls('9', builtInId('<-'))
		.call('10', '{', [argumentInCall('6'), argumentInCall('9')], { origin: ['builtin:expression-list'], returns: ['9'], reads: [builtInId('{')], cds: [{ id: '11', when: true }] })
		.calls('10', builtInId('{'))
...
print(x)`,  emptyGraph()
					...
					.calls('2', builtInId('<-'))
					.argument('2', ['1', '0'])
					.call('7', '<-', [argumentInCall('5'), argumentInCall('6')], { origin: ['builtin:assignment'], returns: ['5'], reads: [builtInId('<-'), 6], onlyBuiltIn: true, environment: defaultEnv().defineVariable('x', '0', '2'), cds: [{ id: '10' }] })
					.calls('7', builtInId('<-'))
					.argument('7', ['6', '5'])
					.call('8', 'break', [], { origin: ['builtin:break'], returns: [], reads: [builtInId('break')], environment: defaultEnv().defineVariable('x', '5', '7'), cds: [{ id: '10' }] })
					.calls('8', builtInId('break'))
					.argument('9', '7')
					.argument('9', '8')
					.call('9', '{', [argumentInCall('7'), argumentInCall('8')], { origin: ['builtin:expression-list'], returns: [], reads: [builtInId('{')], environment: defaultEnv().defineVariable('x', '5', '7'), cds: [{ id: '10' }] })
					.calls('9', builtInId('{'))
					.argument('10', '9')
...
print(x)`, emptyGraph()
					...
					.calls('2',  builtInId('<-'))
					.argument('2', ['1', '0'])
					.call('7', '<-', [argumentInCall('5'), argumentInCall('6')], { origin: ['builtin:assignment'], returns: ['5'], reads: [builtInId('<-'), 6], onlyBuiltIn: true, environment: defaultEnv().defineVariable('x', '0', '2'), cds: [{ id: '13' }, { id: '11', when: true }] })
					.calls('7', builtInId('<-'))
					.argument('7', ['6', '5'])
					.call('9', 'break', [], { origin: ['builtin:break'],  returns: [], reads: [builtInId('break')], cds: [{ id: '13' }, { id: '11', when: true }], environment: defaultEnv().defineVariable('x', '5', '7') })
					.calls('9', builtInId('break'))
					.argument('11', '8')
					.argument('11', '9')
					.call('11', 'if', [argumentInCall('8'), argumentInCall('9', { cds: [{ id: '13' }, { id: '11', when: true }] }), EmptyArgument], { origin: ['builtin:if-then-else'], returns: ['9'], reads: [builtInId('if'), '8'], onlyBuiltIn: true, environment: defaultEnv().defineVariable('x', '5', '7'), cds: [{ id: '13' }, { id: '11', when: true }] })
					.calls('11', builtInId('if'))
					.argument('12', '7')
					.argument('12', '11')
					.call('12', '{', [argumentInCall('7'), argumentInCall('11')], {  origin: ['builtin:expression-list'], returns: ['11'], reads: [builtInId('{')], environment: defaultEnv().defineVariable('x', '5', '7'), cds: [{ id: '13' }, { id: '11', when: true }] })
					.calls('12', builtInId('{'))
					.argument('13', '12')
...
print(x)`,  emptyGraph()
					...
					.calls('2' , builtInId('<-'))
					.argument('2', ['1', '0'])
					.call('7', '<-', [argumentInCall('5'), argumentInCall('6')], { origin: ['builtin:assignment'], returns: ['5'], reads: [builtInId('<-'), 6], onlyBuiltIn: true, environment: defaultEnv().defineVariable('x', '0', '2'), cds: [{ id: '15' }] })
					.calls('7', builtInId('<-'))
					.argument('7', ['6', '5'])
					.call('8', 'next', [], { origin: ['builtin:default'], returns: [], reads: [builtInId('next')], environment: defaultEnv().defineVariable('x', '5', '7'), cds: [{ id: '15' }] })
					.calls('8', builtInId('next'))
					.argument('14', '7')
					.call('14', '{', [argumentInCall('7')], { origin: ['builtin:expression-list'], returns: [], reads: [builtInId('{')], environment: defaultEnv().defineVariable('x', '0', '2'), cds: [{ id: '15' }] })
					.calls('14', builtInId('{'))
					.argument('15', '14')
...
print(x)`, emptyGraph()
					...
					.calls('11', builtInId('<-'))
					.argument('11', ['10', '9'])
					.call('12', 'break', [], { origin: ['builtin:break'], returns: [], reads: [builtInId('break')], cds: [{ id: '14', when: true }], environment: defaultEnv().defineVariable('x', '9', '11', [{ id: '14', when: true }]) })
...
print(x)`,  emptyGraph()
					...
					.calls('11', builtInId('<-'))
					.argument('11', ['10', '9'])
					.call('13', 'break', [], { origin: ['builtin:break'], returns: [], reads: [builtInId('break')], cds: [{ id: '17', when: true }, { id: '15', when: true }], environment: defaultEnv().defineVariable('x', '9', '11', [{ id: '17', when: true }]) })
...
print(x)`,  emptyGraph()
					...
					.calls('8', builtInId('<-'))
					.argument('8', ['7', '6'])
					.call('9', 'break', [], { origin: ['builtin:break'], returns: [], reads: [builtInId('break')], cds: [{ id: 11, when: true }], environment: defaultEnv().defineVariable('x', '0', '2').defineVariable('x', '6', '8', [{ id: '11', when: true }]) })
					.calls('9', builtInId('break'))
					.argument('10', '8')
					.argument('10', '9')
...
print(x)`, emptyGraph()
					...
					.calls('8', builtInId('<-'))
					.argument('8', ['7', '6'])
					.call('10', 'break', [], { origin: ['builtin:break'], returns: [], reads: [builtInId('break')], cds: [{ id: 14, when: true }, { id: '12', when: true }], environment: defaultEnv().defineVariable('x', '0', '2').defineVariable('x', '6', '8', [{ id: '14', when: true }]) })
