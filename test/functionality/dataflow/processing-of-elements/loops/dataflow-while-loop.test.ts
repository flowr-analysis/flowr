...
describe.sequential('While', withShell(shell => {
	...
	assertDataflow(label('assignment in loop body', ['while-loop', 'logical', 'name-normal', ...OperatorDatabase['<-'].capabilities, 'numbers']), shell, 'while (TRUE) { x <- 3 }', emptyGraph()
		.call('5', '<-', [argumentInCall('3'), argumentInCall('4')], { returns: ['3'], reads: [builtInId('<-'), 4], onlyBuiltIn: true, cds: [{ id: 7, when: true }], origin: ['builtin:assignment'] })
		.calls('5', builtInId('<-'))
		.call('6', '{', [argumentInCall('5')], { returns: ['5'], reads: [builtInId('{')], cds: [{ id: 7, when: true }], origin: ['builtin:expression-list'] })
		.calls('6', builtInId('{'))
	...
	assertDataflow(label('def compare in loop', ['while-loop', 'grouping', ...OperatorDatabase['<-'].capabilities, 'name-normal', 'infix-calls', 'binary-operator', ...OperatorDatabase['-'].capabilities, ...OperatorDatabase['>'].capabilities, 'precedence']), shell, 'while ((x <- x - 1) > 0) { x }', emptyGraph()
		...
		.calls('9', builtInId('>'))
		.call('13', '{', [argumentInCall('12')], { returns: ['12'], reads: [builtInId('{')], cds: [{ id: 14, when: true }], environment: defaultEnv().defineVariable('x', '2', '6'), origin: ['builtin:expression-list'] })
		.calls('13', builtInId('{'))
