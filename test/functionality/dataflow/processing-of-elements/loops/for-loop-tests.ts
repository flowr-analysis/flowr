...
describe('for', withShell(shell => {
	assertDataflow('Single-vector for Loop',
		...
		emptyGraph()
			.defineVariable('0', 'i')
			.use('2', 'i', { when: 'maybe', environment: defaultEnvironment().defineVariable('i', '4', '0') })
			.reads('2', '0', 'maybe')
	)

	describe('Potential redefinition with break', () => {
		const withXDefined = defaultEnvironment().defineVariable('x', '2', '0')
		const otherXDefined = defaultEnvironment().defineVariable('x', '9', '7', LocalScope, 'maybe')
...
x`,
	...
	assertDataflow('Read in for Loop',
		...
		emptyGraph()
			...
			.use('7', 'x', { when: 'maybe', environment: envWithX().defineVariable('i', '9', '3') })
			.reads('7', '0', 'maybe')
	)
	const envWithI = () => defaultEnvironment().defineVariable('i', '8', '0')
	assertDataflow('Read after for loop',
		...
		emptyGraph()
			...
			.use('9', 'x', { environment: envWithI().defineVariable('x', '6', '4', LocalScope, 'maybe') })
			.reads('9', '4', 'maybe')
	)


	const envWithFirstX = () => defaultEnvironment().defineVariable('x', '2', '0')
	const envInFor = () => envWithFirstX().defineVariable('i', '11', '3')
	const envOutFor = () => defaultEnvironment().defineVariable('i', '11', '3').defineVariable('x', '2', '0')
	const envWithSecondX = () => defaultEnvironment().defineVariable('x', '9', '7', LocalScope, 'maybe')
	...
	const envInLargeFor = () => envWithFirstX().defineVariable('i', '14', '3')
	const envInLargeFor2 = () => envInLargeFor().defineVariable('x', '9', '7')
	const envOutLargeFor = () => envInLargeFor().defineVariable('x', '12', '10', LocalScope, 'maybe')
	...
	const forLoopWithI = () => defaultEnvironment().defineVariable('i', '9', '0')
	const forLoopWithIAfter = () => defaultEnvironment().defineVariable('i', '9', '0', LocalScope, 'maybe')
	const forLoopAfterI = () => defaultEnvironment().defineVariable('i', '7', '5', LocalScope, 'maybe')
