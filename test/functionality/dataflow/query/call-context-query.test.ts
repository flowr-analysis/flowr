...
describe.sequential('Call Context Query', withShell(shell => {
	...
	describe('Global Targets', () => {
		...
		testQuery('Print calls (global)', 'print(1)', [q(/print/, { callTargets: CallTargets.OnlyGlobal })], r([{ id: 3, calls: [builtInId('print')], name: 'print' }]));
		...
		testQuery('Built-In in Argument', 'print(mean(x))', [q(/mean/, { callTargets: CallTargets.OnlyGlobal })], r([{ id: 4, calls: [builtInId('mean')], name: 'mean' }]));
		testQuery('Multiple Built-In in Argument', 'mean(y)\nprint(mean(x))', [q(/mean/, { callTargets: CallTargets.OnlyGlobal })], r([{ id: 3, calls: [builtInId('mean')], name: 'mean' }, { id: 8, calls: [builtInId('mean')], name: 'mean' }]));
	...
	describe('Mixed Targets', () => {
		...
		testQuery('May be local or global (incl. local)', code, [q(/print/, { callTargets: CallTargets.MustIncludeLocal })], r([{ id: 12, calls: [7, builtInId('print')], name: 'print' }]));
		testQuery('May be local or global (only global)', code, [q(/print/, { callTargets: CallTargets.OnlyGlobal })], baseResult({}));
		testQuery('May be local or global (incl. global)', code, [q(/print/, { callTargets: CallTargets.MustIncludeGlobal })], r([{ id: 12, calls: [7, builtInId('print')], name: 'print' }]));
	});
