...
describe('Query documentation', () => {
	...
	test('every query maps to a distinct wiki page', () => {
		...
		assert.sameMembers(uniqueArray(pages), pages, 'two queries claim the same wiki page');
		...
		assert.sameMembers(uniqueArray(slugs), slugs, 'two query pages share a wiki url');
	});
});
