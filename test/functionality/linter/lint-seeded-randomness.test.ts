...
describe('flowR linter', withTreeSitter(parser => {
	describe('R3 seeded randomness', () => {
		...
		assertLinter('simple no producer', parser, 'runif(1)', 'seeded-randomness',
			[{ range: [1,1,1,8], function: 'runif', certainty: LintingCertainty.Certain }], { consumerCalls: 1, callsWithFunctionProducers: 0, callsWithAssignmentProducers: 0, callsWithNonConstantProducers: 0 });
		...
		assertLinter('condition', parser, 'if(FALSE) { set.seed(17); }\nrunif(1);', 'seeded-randomness',
			[{ range: [2,1,2,8], function: 'runif', certainty: LintingCertainty.Certain }], { consumerCalls: 1, callsWithFunctionProducers: 0, callsWithAssignmentProducers: 0, callsWithNonConstantProducers: 0 });
		assertLinter('condition true', parser, 'if(TRUE) { set.seed(17); }\nrunif(1);', 'seeded-randomness', [], { consumerCalls: 1, callsWithFunctionProducers: 1, callsWithAssignmentProducers: 0, callsWithNonConstantProducers: 0 });

		assertLinter('non-constant seed', parser, 'num<-1 + 7;\nset.seed(num);\nrunif(1);', 'seeded-randomness', [
			{ range: [3,1,3,8], function: 'runif', certainty: LintingCertainty.Certain }
		...
		assertLinter('random seed', parser, 'set.seed(runif(1));\nrunif(1);', 'seeded-randomness', [
			{ range: [1,10,1,17], function: 'runif', certainty: LintingCertainty.Certain },
			{ range: [2,1,2,8], function: 'runif', certainty: LintingCertainty.Certain }
		...
		assertLinter('custom set.seed', parser, 'set.seed <- function(x) {}\nset.seed(17)\nrunif(1)', 'seeded-randomness',
			[{ range: [3,1,3,8], function: 'runif', certainty: LintingCertainty.Certain }], { consumerCalls: 1, callsWithFunctionProducers: 0, callsWithAssignmentProducers: 0, callsWithNonConstantProducers: 0 });
		...
		assertLinter('set in function call', parser, 'runif(set.seed(17))', 'seeded-randomness',
			[{ range: [1,1,1,19], function: 'runif', certainty: LintingCertainty.Certain }]);
		assertLinter('get in function call', parser, 'runif(runif(1))', 'seeded-randomness', [
			{ range: [1,7,1,14], function: 'runif', certainty: LintingCertainty.Certain },
			{ range: [1,1,1,15], function: 'runif', certainty: LintingCertainty.Certain }]);
	});
}));
