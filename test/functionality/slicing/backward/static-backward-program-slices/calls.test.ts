...
a()`, { minRVersion: MIN_VERSION_LAMBDA });
		...
		describe('DelayedAssign', () => {
			...
			function delayedCase(name: string, code: string, criterion: SlicingCriterion, expected: string, out: string) {
				assertSliced(label(name, ['name-normal', 'numbers', 'assignment-functions', 'strings', 'newlines', 'global-scope']),
					shell, code, [criterion], expected, { expectedOutput: out, expectedSliceOutput: out });
			}
...
f(21)`, { includeCallees: true }
	...
	describe('User-defined replacement functions', () => {
		...
		assertSliced(label('a super-assigning user replacement binds outside the frame', [...replCaps, ...OperatorDatabase['<<-'].capabilities, 'side-effects-in-function-call']),
			shell, '`s<-` <- function(x, value) x + value\ny <- 1\nf <- function() { s(y) <<- 5 }\nf()\nv <- y\nv', ['6@v'], '`s<-` <- function(x, value) x + value\ny <- 1\nf <- function() s(y) <<- 5\nf()\nv <- y\nv', { expectedOutput: '[1] 6', expectedSliceOutput: '[1] 6' });
	});
}));
