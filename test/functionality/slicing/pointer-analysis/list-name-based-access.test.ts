...
describe.sequential('List name based access', withShell(shell => {
	...
	describe('Simple access', () => {
		assertSliced(
			label('List with single element', basicCapabilities),
			shell,
		...
		assertSliced(
			label('List with several elements', basicCapabilities),
			shell,
	...
	describe('Whole list access', () => {
		assertSliced(
			label('When each argument of a list is redefined, then original list is still in slice', basicCapabilities),
			shell,
...
print(person)`
		...
		assertSliced(
			label('When arguments are added to an empty list, then original list is in slice', basicCapabilities),
			shell,
			`person <- list()
person$is_male <- FALSE
person$name <- "Alice"
...
print(person)`
		...
		assertSliced(
			label('When whole list is redefined, then every list assignment before is not in slice', basicCapabilities),
			shell,
...
result <- person`
	...
	describe('Access with other accesses', () => {
		assertSliced(
			label('With other list', basicCapabilities),
			shell,
			`person <- list(age = 24, name = "John")
...
print(person$name)`,
		...
		assertSliced(
			label('When there are assignments to only other indices, then only list is in the slice', basicCapabilities),
			shell,
			`person <- list(age = 24, name = "John", is_male = TRUE)
person$is_male <- FALSE
person$name <- "Alice"
...
result <- person$name`,
	...
	describe('Config flag', () => {
		...
		assertSliced(
			label('When flag is false, then list access is not in slice', ['call-normal']),
			shell,
			`person <- list(age = 24, name = "John")
person$name <- "Jane"
person$age <- 23
