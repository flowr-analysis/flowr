...
describe.sequential('Alias Tracking', withShell(shell => {
	test.each([
		...
		const values = trackAliasInEnvironments(defaultConfigOptions.solver.variables, identifier as Identifier, result.dataflow.environment, result.dataflow.builtInEnvironment, result.dataflow.graph, result.dataflow.graph.idMap);
