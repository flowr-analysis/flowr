...
a(i)`
		for(const criterion of ['3:1', '3@a'] as const) {
			assertSliced(label(JSON.stringify(code), ['function-definitions', 'formals-named', 'name-normal', 'call-normal', ...OperatorDatabase['<-'].capabilities, 'unnamed-arguments']),
				shell, code, [criterion], code
			)
		}
		const constCapabilities: SupportedFlowrCapabilityId[] = ['function-definitions', 'formals-named', 'name-normal', 'numbers', 'call-normal', ...OperatorDatabase['<-'].capabilities, 'unnamed-arguments', 'implicit-return']
...
foo(x, x.(3))`)
		assertSliced(label('Using ...', ['name-normal', 'unnamed-arguments', 'formals-dot-dot-dot', 'formals-named', 'implicit-return', 'call-normal', ...OperatorDatabase['<-'].capabilities, 'newlines', 'numbers']),
			shell, `
f1 <- function (a,b) { c }
f2 <- function (...) { f1(...) }
x <- 3
...
a(4)`
			for(const criterion of ['3:1', '3@a'] as const) {
				assertSliced(label('Must include read', ['name-normal', 'unnamed-arguments', 'formals-named', 'implicit-return', 'call-normal', ...OperatorDatabase['<-'].capabilities, 'newlines', 'binary-operator', 'infix-calls', ...OperatorDatabase['+'].capabilities, 'numbers']),
					shell, code, [criterion], code)
			}
		})
...
a(5)`
			for(const criterion of ['4:1', '4@a'] as const) {
				assertSliced(label('Only keep second definition', ['name-normal', 'unnamed-arguments', 'formals-named', 'implicit-return', 'call-normal', ...OperatorDatabase['<-'].capabilities, 'newlines', 'binary-operator', 'infix-calls', ...OperatorDatabase['+'].capabilities, 'numbers']), shell, code, [criterion], `a <- function(x) { x + i }
i <- 4
a(5)`)
			}
		})
...
a(x = 3)`
		assertSliced(label('Must include function definition', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'formals-default', 'implicit-return', 'newlines', 'named-arguments', 'numbers']),
			shell, code, ['2@a'], code)
...
f()
`
		assertSliced(label('Late bindings of parameter in body', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'formals-default', 'numbers', 'implicit-return', 'binary-operator', 'infix-calls', ...OperatorDatabase['+'].capabilities, 'call-normal', 'semicolons']),
...
f()
`
		assertSliced(label('Late bindings of parameter in parameters', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'formals-default', 'newlines','binary-operator', 'infix-calls', 'numbers', 'call-normal', ...OperatorDatabase['+'].capabilities, 'semicolons']),
			shell, lateCodeB, ['2@f'], `f <- function(a=b, b=3) { a + 1 }
f()`)
		assertSliced(label('Parameters binding context', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'formals-default', 'implicit-return', 'newlines', 'numbers', 'call-normal']),
...
b <- a()
b()`
			assertSliced(label('Must include outer function', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'normal-definition', 'numbers', 'return', 'implicit-return', 'call-normal', 'newlines', 'semicolons']),
...
u <- a()
u()`
			assertSliced(label('Must include function shell', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'normal-definition', 'implicit-return', 'numbers', 'binary-operator', 'infix-calls', ...OperatorDatabase['+'].capabilities, 'return', 'newlines', 'call-normal', 'semicolons']),
				shell, code, ['5@a'], `a <- function() {
        x <- function() { }
        return(x)
    }
a()`)
			assertSliced(label('Must include function shell on call', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'normal-definition', 'newlines', 'return', 'call-normal']), shell, code, ['6@u'], `a <- function() {
        x <- function() { z + y }
        y <- 12
        return(x)
    }
...
b <- function(f) { i <- 5; f() }
b(a)`
		const caps: SupportedFlowrCapabilityId[] = ['name-normal', ...OperatorDatabase['<-'].capabilities, 'normal-definition', 'implicit-return', 'newlines', 'numbers', 'formals-named', 'call-normal', 'unnamed-arguments']
...
b(a)`)
		assertSliced(label('Directly call returned function', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'formals-named', 'normal-definition', 'implicit-return', 'return', 'unnamed-arguments', 'newlines', 'numbers', 'call-normal']),
			shell, `m <- 12
a <- function(x) {
...
res <- a(m)()`)
		assertSliced(label('Higher order anonymous function', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'formals-named', 'implicit-return', 'normal-definition', 'call-anonymous', 'binary-operator', 'infix-calls', ...OperatorDatabase['+'].capabilities, 'newlines']),
			shell, `a <- function(b) {
  b
}
...
res <- x()`
		assertSliced(label('Double return points', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'call-anonymous', 'normal-definition', 'implicit-return', 'numbers', 'if', 'return', 'implicit-return', 'call-normal', 'newlines']), shell, code, ['9@res'], `
x <- (function() {
        g <- function() { y }
        y <- 5
        if(z) return(g)
...
a()
cat(x)`
		const localCaps: SupportedFlowrCapabilityId[] = ['name-normal', 'normal-definition', ...OperatorDatabase['='].capabilities, 'binary-operator', 'infix-calls', ...OperatorDatabase['+'].capabilities, 'semicolons', 'unnamed-arguments', 'newlines', 'call-normal', 'numbers']
...
cat(x)`)
		...
		assertSliced(label('Must work with nested globals', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'normal-definition', 'formals-named', 'side-effects-in-function-call', 'return-value-of-assignments', 'newlines', 'numbers', 'call-normal', 'unnamed-arguments']),
...
cat(x)`)
		...
		assertSliced(label('Must work with nested globals and known assignments not-happening', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'normal-definition', 'formals-named', 'if', 'logical', ...OperatorDatabase['<<-'].capabilities, 'return-value-of-assignments', 'implicit-return', 'newlines', 'call-normal', 'unnamed-arguments']),
...
cat(x)`)
		...
		assertSliced(label('Must work with nested globals and maybe assignments', ['name-normal', ...OperatorDatabase['<-'].capabilities, 'normal-definition', 'formals-named', 'if', 'call-normal', ...OperatorDatabase['>'].capabilities, 'numbers', ...OperatorDatabase['<<-'].capabilities, 'return-value-of-assignments', 'newlines', 'unnamed-arguments']),
...
\`a\`()
    `
		const caps: SupportedFlowrCapabilityId[] = ['name-quoted', ...OperatorDatabase['<-'].capabilities, 'normal-definition', 'name-normal', 'numbers', 'semicolons', 'implicit-return', 'call-normal', 'newlines', 'name-escaped']
...
cat(4 %a% 5)
      `
		assertSliced(label('Must link alias with namespace', ['accessing-exported-names', 'name-quoted', ...OperatorDatabase['<-'].capabilities, 'formals-named', 'implicit-return', 'binary-operator', 'infix-calls', ...OperatorDatabase['+'].capabilities, 'special-operator', 'unnamed-arguments']),
