...
import { RLogicalType, RTypeUnion, RNullType } from '../../../../src/typing/types';
...
describe('Infer types for loops', () => {
	describe.each([
		...
		{ input: 'while(TRUE) { next }',                                              expectedType: new RTypeUnion() },
		/* the condition folds to `TRUE`, so the loop never leaves and stands for no value */
		{ input: 'while(1 < 2) { print("Hi Flo!") }',                                 expectedType: new RTypeUnion() },
		...
		{ input: 'repeat { print("I love testing!") }',                               expectedType: new RTypeUnion() },
