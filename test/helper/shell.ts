	...
	return it(`${JSON.stringify(criteria)} ${name}`, async function() {
		...
		try {
			assert.strictEqual(
				...
				`got: ${result.reconstruct.code}, vs. expected: ${expected}, for input ${input} (slice: ${printIdMapping(result.slice.decodedCriteria.map(({ id }) => id), result.normalize.idMap)}), url: ${graphToMermaidUrl(result.dataflow.graph, result.normalize.idMap, result.slice.result)}`
			)
		} catch(e) {
