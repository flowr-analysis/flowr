...
describe('Objects', () => {
  describe('isObjectOrArray', () => {
    const positive = (a: any, msg: string): void => { assert.isTrue(isObjectOrArray(a), `must hold for ${msg}`) }
    const negative = (a: any, msg: string): void => { assert.isFalse(isObjectOrArray(a), `${msg} is not considered an object`) }
