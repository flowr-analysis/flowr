    ...
    describe('guard against illegal parameters', () => {
      it('negative', function () {
        for (const length of [-Infinity, -42, -2, -1]) {
          assert.throws(() => randomString(length), Error, undefined, `length must be positive (${length} >= 0)`)
        }
      })
      it('NaN and Infinity', function () {
        for (const length of [Infinity, NaN]) {
          assert.throws(() => randomString(length), Error, undefined, `length must be neither NaN nor Infinity (${length}`)
        }
      })
      it('Floating Point', function () {
        for (const length of [2.3, 42.42, Math.PI]) {
          assert.throws(() => randomString(length), Error, undefined, `length must be an integer (${length}`)
        }
      })
      it('empty source', function () {
        assert.throws(() => randomString(42, []), Error, undefined, 'source must not be empty')
      })
    })
  })
})
