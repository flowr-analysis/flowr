_This document was generated from '[src/documentation/print-cfg-wiki.ts](https://github.com/flowr-analysis/flowr/tree/main//src/documentation/print-cfg-wiki.ts)' on 2025-05-26, 10:38:45 UTC presenting an overview of flowR's control flow graph (v2.2.12, using R v4.5.0). Please do not edit this file/wiki page directly._
...
The analysis required _9.6 ms_ (including the dataflow analysis,  normalization,  and parsing with the [r-shell](https://github.com/flowr-analysis/flowr/wiki/Engines) engine) within the generation environment.
We used the following simplification: `unique-cf-sets` .
...
The analysis required _1.1 ms_ (including the  normalization and parsing with the [r-shell](https://github.com/flowr-analysis/flowr/wiki/Engines) engine) within the generation environment.
We used the following simplification: `unique-cf-sets` .
...
- [<code><span title="This event triggers for every function call that is not handled by a specific overload, and hence may be a function that targets a user-defined function. Use SemanticCfgGuidedVisitor#getOrigins to get the origins of the call.  For example, this triggers for foo(x) in    foo <- function(x) { x + 1 } foo(x)  ">SemanticCfgGuidedVisitor::<b>onDefaultFunctionCall</b></span></code>](https://github.com/flowr-analysis/flowr/tree/main//src/control-flow/semantic-cfg-guided-visitor.ts#L359)\
  ...
  Use
  <code>SemanticCfgGuidedVisitor#getOrigins</code>
...
- [<code><span title="Called for every anonymous function definition.  For example, function(x) { x + 1 } in lapply(1:10, function(x) { x + 1 }).">SemanticCfgGuidedVisitor::<b>onFunctionDefinition</b></span></code>](https://github.com/flowr-analysis/flowr/tree/main//src/control-flow/semantic-cfg-guided-visitor.ts#L334)\
  Called for every anonymous function definition.
  
  For example, `function(x) { x + 1 }` in `lapply(1:10, function(x) { x + 1 })`.

- [<code><span title="This event triggers for every call to the get function, which is used to access variables in the global environment.  For example, get in get('x').">SemanticCfgGuidedVisitor::<b>onGetCall</b></span></code>](https://github.com/flowr-analysis/flowr/tree/main//src/control-flow/semantic-cfg-guided-visitor.ts#L430)\
...
- [<code><span title="This event triggers for every call to the if function, which is used to implement the if-then-else control flow.">SemanticCfgGuidedVisitor::<b>onIfThenElseCall</b></span></code>](https://github.com/flowr-analysis/flowr/tree/main//src/control-flow/semantic-cfg-guided-visitor.ts#L420)\
...
- [<code><span title="Called for every occurrence of a NULL in the program.">SemanticCfgGuidedVisitor::<b>onNullConstant</b></span></code>](https://github.com/flowr-analysis/flowr/tree/main//src/control-flow/semantic-cfg-guided-visitor.ts#L289)\
  Called for every occurrence of a `NULL` in the program.

- [<code><span title="Called for every constant number value in the program.  For example, 42 in print(42).">SemanticCfgGuidedVisitor::<b>onNumberConstant</b></span></code>](https://github.com/flowr-analysis/flowr/tree/main//src/control-flow/semantic-cfg-guided-visitor.ts#L303)\
  Called for every constant number value in the program.
  
  For example, `42` in `print(42)`.

- [<code><span title="This event triggers for every call to R's pipe operator, i.e., for every call to |>.">SemanticCfgGuidedVisitor::<b>onPipeCall</b></span></code>](https://github.com/flowr-analysis/flowr/tree/main//src/control-flow/semantic-cfg-guided-visitor.ts#L479)\
  This event triggers for every call to R's pipe operator, i.e., for every call to `|>`.

- [<code>onProgram</code>](https://github.com/flowr-analysis/flowr/tree/main//src/control-flow/semantic-cfg-guided-visitor.ts#L277)\
  

- [<code><span title="This event triggers for every call to the quote function, which is used to quote expressions.  For example, quote in quote(x + 1).  More specifically, this relates to the corresponding BuiltInProcessorMapper handler.">SemanticCfgGuidedVisitor::<b>onQuoteCall</b></span></code>](https://github.com/flowr-analysis/flowr/tree/main//src/control-flow/semantic-cfg-guided-visitor.ts#L491)\
...
- [<code><span title="This event triggers for every call to the rm function, which is used to remove variables from the environment.  For example, rm in rm(x).">SemanticCfgGuidedVisitor::<b>onRmCall</b></span></code>](https://github.com/flowr-analysis/flowr/tree/main//src/control-flow/semantic-cfg-guided-visitor.ts#L439)\
...
- [<code><span title="Called for every constant string value in the program.  For example, 'Hello World' in print('Hello World').">SemanticCfgGuidedVisitor::<b>onStringConstant</b></span></code>](https://github.com/flowr-analysis/flowr/tree/main//src/control-flow/semantic-cfg-guided-visitor.ts#L296)\
  Called for every constant string value in the program.
  
  For example, `"Hello World"` in `print("Hello World")`.

- [<code><span title="This event triggers for every anonymous call within the program.  For example, (function(x) { x + 1 })(42) or the second call in a()().">SemanticCfgGuidedVisitor::<b>onUnnamedCall</b></span></code>](https://github.com/flowr-analysis/flowr/tree/main//src/control-flow/semantic-cfg-guided-visitor.ts#L343)\
  This event triggers for every anonymous call within the program.
  
  For example, `(function(x) { x + 1 })(42)` or the second call in `a()()`.

- [<code><span title="Called for every variable that is written within the program. You can use getOrigins to get the origins of the variable.  For example, x in x <- 42 or x in assign('x', 42). See SemanticCfgGuidedVisitor#onAssignmentCall for the assignment call. This event handler also provides you with informaton on the source.">SemanticCfgGuidedVisitor::<b>onVariableDefinition</b></span></code>](https://github.com/flowr-analysis/flowr/tree/main//src/control-flow/semantic-cfg-guided-visitor.ts#L327)\
