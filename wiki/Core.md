_This document was generated from '[src/documentation/wiki-core.ts](https://github.com/flowr-analysis/flowr/tree/main//src/documentation/wiki-core.ts)' on 2026-07-29, 07:17:39 UTC presenting an overview of flowR's core (v2.13.4, using R v4.6.1). Please do not edit this file/wiki page directly._
...
> flowR repl v2.13.4, R v4.6.1 (r-shell engine)
> R> :parse "x <- 1; print(x)"
> ```
> 
> <details>
...
1. **<a href="https://github.com/flowr-analysis/flowr/tree/main/src/core/steps/all/core/00-parse.ts#L8">PARSE_WITH_R_SHELL_STEP</a>** ([parsing](#parsing)): Uses the <a href="https://github.com/flowr-analysis/flowr/tree/main/src/r-bridge/shell.ts#L143"><code><span title="The RShell represents an interactive session with the R interpreter. You can configure it by RShellOptions . At the moment we are using a live R session (and not networking etc.) to communicate with R easily, which allows us to install packages etc. However, this might and probably will change in the future (leaving this as a legacy mode :D)">RShell</span></code></a> to parse the input program.\
   _Its main function linked as the processor is the <a href="https://github.com/flowr-analysis/flowr/tree/main/src/r-bridge/parser.ts#L88"><span title="Takes an input program and parses it using the given parser.">parseRequests</span></a> function._
...
(The analysis required _7.1 ms_ (including parsing with the [r-shell](https://github.com/flowr-analysis/flowr/wiki/Engines) engine) within the generation environment.)
...
> ```
> 	
> (The analysis required _6.4 ms_ (including parsing with the [tree-sitter](https://github.com/flowr-analysis/flowr/wiki/Engines) engine) within the generation environment.)
> 
> 
> 
> </details>
...
we do not care about this node and just produce an empty dataflow information (using <a href="https://github.com/flowr-analysis/flowr/tree/main/src/dataflow/info.ts#L197"><code><span title="Initializes an empty DataflowInformation object with the given entry point and data. This is to be used as a 'starting point' when processing leaf nodes during the dataflow extraction.">DataflowInformation::<b>initialize</b></span></code></a>). 
...
FlowR's heart and soul resides in the <a href="https://github.com/flowr-analysis/flowr/tree/main/src/dataflow/environments/default-builtin-config.ts#L149"><code><span title="Contains the built-in definitions recognized by flowR">DefaultBuiltinConfig</span></code></a> object, which is used to configure the built-in environment
by mapping function names to <a href="https://github.com/flowr-analysis/flowr/tree/main/src/dataflow/environments/built-in.ts#L243"><code>BuiltInProcessorMapper</code></a> functions.
There you can find functions like <a href="https://github.com/flowr-analysis/flowr/tree/main/src/dataflow/internal/process/functions/call/built-in/built-in-access.ts#L50"><code><span title="Processes different types of access operations.  Example:   a[i] a$foo a[[i]] a@foo  ">processAccess</span></code></a> which handles the (subset) access to a variable,
...
Besides the processor, an entry states what the function does with <a href="https://github.com/flowr-analysis/flowr/tree/main/src/dataflow/environments/built-in-props.ts#L132"><code><span title="Semantics of a built-in that hold no matter which processor handles the call. The remaining facts already have a home: the exit behavior in cfg, whether flowR can fold the call in the evalHandler of the definition, and the fallback for everything unmodelled in hasUnknownSideEffects.">BuiltInFnInfo</span></code></a>: its <a href="https://github.com/flowr-analysis/flowr/tree/main/src/dataflow/environments/built-in-props.ts#L35"><code><span title="What the call as a whole does, as a bitmask. The resource bits ( CallProp.File and its neighbors) say where the call gets its data from, which is what InputProps collects.">CallProp</span></code></a> bits
say what the call as a whole does, its <a href="https://github.com/flowr-analysis/flowr/tree/main/src/dataflow/environments/built-in-props.ts#L125"><code><span title="The formals of a built-in, in the order they are declared in, each with what its argument is used for. A ... entry stands for every argument from the position it appears at, and the entries behind it are matched by their full name only.">FnSig</span></code></a> says what each argument is used for (in the order R declares them,
with `...` covering every position from where it appears). Anything you add there is picked up by <a href="https://github.com/flowr-analysis/flowr/tree/main/src/dataflow/environments/query-fn-props.ts#L133"><code><span title="Every built-in whose props carry any of props, as the identifiers they are registered under. The answer for the DefaultBuiltinConfig is computed on first use and cached.">builtInsWith</span></code></a>,
which is how the [input-sources query](https://github.com/flowr-analysis/flowr/wiki/query-api) learns which functions bring in data of their own.

Hover a name for what it means, <sup>db</sup> marks the ones the [signature database](https://github.com/flowr-analysis/flowr/wiki/signature-database) states for
any package function (<a href="https://github.com/flowr-analysis/flowr/tree/main/src/dataflow/environments/query-fn-props.ts#L84"><code><span title="What the signature database implies for a package function: what its own entry states (see fnInfoFromSignature ) plus the PropagatedProps of everything it calls, transitively. A package function that ends up in system() runs a system command as well.">inferFnProps</span></code></a> reads them off, and carries what a function calls over to it):

* <a href="https://github.com/flowr-analysis/flowr/tree/main/src/dataflow/environments/built-in-props.ts#L35"><code><span title="What the call as a whole does, as a bitmask. The resource bits ( CallProp.File and its neighbors) say where the call gets its data from, which is what InputProps collects.">CallProp</span></code></a>: <span title="computes a result and nothing else, the positive counterpart of `hasUnknownSideEffects`">`Pure` 286</span> &middot; <span title="pure on its own, but it runs code it is handed, so whatever that code does happens too. The parameter it runs is marked ArgProp.Callee or ArgProp.Nse , as with `lapply(x, f)`.">`MayPure` 30</span> &middot; <span title="may signal an error, like `stop()` (see SigDbInferable )">`Throws`<sup>db</sup> 10</span> &middot; <span title="returns invisibly, so the result is not auto-printed">`Invisible` 8</span> &middot; <span title="dispatches on the class of its first argument (S3, S4, or S7)">`Generic`<sup>db</sup> 3</span> &middot; <span title="binds, rebinds, or removes names outside of its own frame, like `assign` or `library`">`Scope` 55</span> &middot; <span title="draws from the random number generator, or sets its state">`Random` 28</span> &middot; <span title="depends on ambient state like the clock, the locale, environment variables, or global options">`Ambient` 14</span> &middot; <span title="touches the file system">`File` 77</span> &middot; <span title="produces a temporary path, the narrower case of `File`">`TempFile` 8</span> &middot; <span title="always reaches the network, like `curl::curl_download`. Calls that only do so for some arguments, like `read.csv` of a URL, are left to the `network-functions` rule, which decides that per call site.">`Network` 21</span> &middot; <span title="runs a system command">`Process` 6</span> &middot; <span title="calls native code through the foreign function interface, like `.Call`">`Ffi` 7</span> &middot; <span title="produces a language object, like `quote` or `deparse`">`Lang` 46</span> &middot; <span title="asks the user, like `readline` or a file chooser">`User` 44</span> &middot; <span title="draws on a graphics device">`Graphics` 513</span> &middot; <span title="reads the resource its `Resource` arguments name">`Reads` 22</span> &middot; <span title="writes the resource its `Resource` arguments name">`Writes` 63</span> &middot; <span title="may emit to standard output, like `print` or a `cat` without a `file`, and follows a `sink` when one is active">`Prints` 10</span>
* <a href="https://github.com/flowr-analysis/flowr/tree/main/src/dataflow/environments/built-in-props.ts#L7"><code><span title="What a single argument of a call is used for, as a bitmask.">ArgProp</span></code></a>: <span title="the result is this argument, handed back unchanged, like `x` in `identity(x)`">`Alias` 3</span> &middot; <span title="the result is computed from the argument's value, like `x` in `sum(x)`">`Value` 144</span> &middot; <span title="only the shape is used (length, dimensions, names, other attributes), like `x` in `nrow(x)`">`Shape` 23</span> &middot; <span title="selects a behavior instead of carrying data, like `na.rm` in `sum(x, na.rm = TRUE)`">`Flag` 2</span> &middot; <span title="names the resource the call reads or writes, like `file` in `write.csv(x, file)`">`Resource` 48</span> &middot; <span title="what it refers to may be modified, like `envir` in `assign(x, v, envir = e)`">`Written` 1</span> &middot; <span title="evaluated whenever the call happens, even if the result goes unused, like `x` in `force(x)`">`Forced` 4</span> &middot; <span title="quoted or evaluated in another frame, like `expr` in `quote(expr)`">`Nse` 6</span> &middot; <span title="called as a function, like `FUN` in `lapply(x, FUN)`">`Callee` 13</span> &middot; <span title="only whether it was supplied matters, as with `missing()`">`Presence` 1</span>
...
<a href="https://github.com/flowr-analysis/flowr/tree/main/src/dataflow/internal/linker.ts#L557"><code><span title="all loops variables which are open read (not already bound by a redefinition within the loop) get a maybe read marker to their last definition within the loop e.g. with:   for(i in 1:10) {  x_1 <- x_2 + 1 }   x_2 must get a read marker to x_1 as x_1 is the active redefinition in the second loop iteration.  When environment is supplied the function uses it to discover ALL definitions that are still...">linkCircularRedefinitionsWithinALoop</span></code></a> and then retrieve the active exit points with <a href="https://github.com/flowr-analysis/flowr/tree/main/src/dataflow/info.ts#L271"><code><span title="Filters out exit points which end their cascade within a loop.">filterOutLoopExitPoints</span></code></a>.
...
Your main point of interesting here is the <a href="https://github.com/flowr-analysis/flowr/tree/main/src/slicing/static/static-slicer.ts#L60"><code><span title="Computes the node ids to include in a static slice, starting from the given seed ids. The returned ids can be used with reconstructToCode to reproduce executable R code.">staticSlice</span></code></a> function which relies on a modified
...
Any analysis site queries the level with <a href="https://github.com/flowr-analysis/flowr/tree/main/src/project/context/flowr-analyzer-gas-context.ts#L94"><code>FlowrAnalyzerGasContext::<b>checkGas</b></code></a>, where `key` is a feature name.
The call is a no-op when gas is disabled for that key and no gas plugins are registered.
Heap statistics come from the `v8` module (Node.js, Electron, VS Code) or Chromium's `performance.memory` in browsers. If neither is available, gas skips the memory check and only the elapsed-time thresholds apply. Programmatic configs can supply a custom source via the `heapProvider` of the <a href="https://github.com/flowr-analysis/flowr/tree/main/src/gas.ts#L72"><code><span title="Gas configuration embedded in FlowrConfig.gas . Each entry in features is a sensitivity factor for a named analysis feature (see GasFeatureKey for the recognised keys): - 0 / absent - disabled for that feature, zero overhead. - 1 - normal sensitivity, thresholds in thresholds apply as-is. - N - N times as sensitive, effectively divides each threshold by N. See the [Gas (Resource Guard)](https://gi...">FlowrGasConfig</span></code></a> (`config.gas.heapProvider`), and gas plugins can override levels entirely.
