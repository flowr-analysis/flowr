_This document was generated from '[src/documentation/wiki-core.ts](https://github.com/flowr-analysis/flowr/tree/main//src/documentation/wiki-core.ts)' on 2026-07-29, 04:39:03 UTC presenting an overview of flowR's core (v2.13.4, using R v4.6.1). Please do not edit this file/wiki page directly._
...
* <a href="https://github.com/flowr-analysis/flowr/tree/main/src/dataflow/environments/built-in-props.ts#L35"><code><span title="What the call as a whole does, as a bitmask. The resource bits ( CallProp.File and its neighbors) say where the call gets its data from, which is what InputProps collects.">CallProp</span></code></a>: <span title="computes a result and nothing else, the positive counterpart of `hasUnknownSideEffects`">`Pure` 288</span> &middot; <span title="pure on its own, but it runs code it is handed, so whatever that code does happens too. The parameter it runs is marked ArgProp.Callee or ArgProp.Nse , as with `lapply(x, f)` (see SigDbInferable ).">`MayPure`<sup>db</sup> 30</span> &middot; <span title="may signal an error, like `stop()` (see SigDbInferable )">`Throws`<sup>db</sup> 10</span> &middot; <span title="may signal a warning or another condition, like `warning()`">`Warns` 5</span> &middot; <span title="returns invisibly, so the result is not auto-printed">`Invisible` 8</span> &middot; <span title="dispatches on the class of its first argument (S3, S4, or S7)">`Generic` 3</span> &middot; <span title="binds, rebinds, or removes names outside of its own frame, like `assign` or `library`">`Scope` 54</span> &middot; <span title="draws from the random number generator, or sets its state">`Random` 3</span> &middot; <span title="depends on ambient state like the clock, the locale, environment variables, or global options">`Ambient` 9</span> &middot; <span title="touches the file system">`File` 48</span> &middot; <span title="produces a temporary path, the narrower case of `File`">`TempFile` 8</span> &middot; <span title="always reaches the network, like `curl::curl_download`. Calls that only do so for some arguments, like `read.csv` of a URL, are left to the `network-functions` rule, which decides that per call site.">`Network` 18</span> &middot; <span title="runs a system command">`Process` 6</span> &middot; <span title="calls native code through the foreign function interface, like `.Call`">`Ffi` 7</span> &middot; <span title="produces a language object, like `quote` or `deparse`">`Lang` 46</span> &middot; <span title="asks the user, like `readline` or a file chooser">`User` 44</span> &middot; <span title="draws on a graphics device">`Graphics` 514</span> &middot; <span title="reads the resource its `Resource` arguments name">`Reads` 4</span> &middot; <span title="writes the resource its `Resource` arguments name">`Writes` 51</span>
* <a href="https://github.com/flowr-analysis/flowr/tree/main/src/dataflow/environments/built-in-props.ts#L7"><code><span title="What a single argument of a call is used for, as a bitmask.">ArgProp</span></code></a>: <span title="the result is this argument, handed back unchanged, like `x` in `identity(x)`">`Alias` 3</span> &middot; <span title="the result is computed from the argument's value, like `x` in `sum(x)`">`Value` 138</span> &middot; <span title="only the shape is used (length, dimensions, names, other attributes), like `x` in `nrow(x)`">`Shape` 23</span> &middot; <span title="selects a behavior instead of carrying data, like `na.rm` in `sum(x, na.rm = TRUE)`">`Flag` 3</span> &middot; <span title="names the resource the call reads or writes, like `file` in `write.csv(x, file)`">`Resource` 20</span> &middot; <span title="what it refers to may be modified, like `envir` in `assign(x, v, envir = e)`">`Written` 2</span> &middot; <span title="evaluated whenever the call happens, even if the result goes unused, like `x` in `force(x)`">`Forced` 4</span> &middot; <span title="quoted or evaluated in another frame, like `expr` in `quote(expr)`">`Nse` 5</span> &middot; <span title="called as a function, like `FUN` in `lapply(x, FUN)`">`Callee` 13</span> &middot; <span title="only whether it was supplied matters, as with `missing()`">`Presence` 1</span>
