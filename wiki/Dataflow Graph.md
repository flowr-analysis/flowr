_This document was generated from '[src/documentation/print-dataflow-graph-wiki.ts](https://github.com/flowr-analysis/flowr/tree/main//src/documentation/print-dataflow-graph-wiki.ts)' on 2025-02-03, 08:25:34 UTC presenting an overview of flowR's dataflow graph (v2.2.1, using R v4.4.2). Please do not edit this file/wiki page directly._
...
The analysis required _17.37 ms_ (including parse and normalize) within the generation environment. 
...
The analysis required _2.40 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {0}.
We encountered no unknown side effects during the analysis.
...
The analysis required _6.21 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {1}.
We encountered no unknown side effects during the analysis.
...
The analysis required _1.83 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {0}.
We encountered no unknown side effects during the analysis.
...
The analysis required _12.66 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {1}.
We encountered no unknown side effects during the analysis.
...
The analysis required _7.75 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {3, 0->3}.
...
The analysis required _6.85 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {10, 10->0, 10->4}.
...
The analysis required _9.31 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {11, 11->0, 11->5}.
...
The analysis required _9.11 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {16, 16->1, 16->7}.
...
The analysis required _1.90 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {1}.
We encountered no unknown side effects during the analysis.
...
The analysis required _21.62 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {8}.
We encountered no unknown side effects during the analysis.
...
> <summary style="color:gray">R Code of the Dataflow Graph</summary>
> 
> The analysis required _5.84 ms_ (including parse and normalize) within the generation environment. 
> We encountered no unknown side effects during the analysis.
> 
> ```r
> x <- 2
...
> <summary style="color:gray">R Code of the Dataflow Graph</summary>
> 
> The analysis required _4.94 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {6, 6->0, 6->1, 6->3}.
> We encountered no unknown side effects during the analysis.
> 
> ```r
> foo <- function() 3
...
> <summary style="color:gray">R Code of the Dataflow Graph</summary>
> 
> The analysis required _4.90 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {9, 9->5, 9->3}.
> We encountered no unknown side effects during the analysis.
> 
> ```r
> f <- function() 3
...
> <summary style="color:gray">R Code of the Dataflow Graph</summary>
> 
> The analysis required _7.97 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {22, 22->18}.
> We encountered no unknown side effects during the analysis.
> 
> ```r
> f <- function() {
...
> 
> The analysis required _5.98 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {9, 9->0, 9->10}.
> We encountered no unknown side effects during the analysis.
> 
> 
> ```mermaid
...
The analysis required _3.80 ms_ (including parse and normalize) within the generation environment. 
We encountered no unknown side effects during the analysis.
...
The analysis required _4.15 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {6, 6->4}.
We encountered no unknown side effects during the analysis.
...
The analysis required _3.64 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {12, 12->4}.
We encountered no unknown side effects during the analysis.
...
The analysis required _3.37 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {8, 1->8}.
We encountered no unknown side effects during the analysis.
...
The analysis required _2.25 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {0}.
We encountered no unknown side effects during the analysis.
...
The analysis required _1.44 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {0}.
We encountered no unknown side effects during the analysis.
...
The analysis required _2.25 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {1}.
We encountered no unknown side effects during the analysis.
...
The analysis required _9.35 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {0}.
We encountered no unknown side effects during the analysis.
...
The analysis required _4.16 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {0}.
We encountered no unknown side effects during the analysis.
...
The analysis required _4.47 ms_ (including parse and normalize) within the generation environment. 
We encountered no unknown side effects during the analysis.
...
The analysis required _2.37 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {2}.
We encountered no unknown side effects during the analysis.
...
> <summary style="color:gray">R Code of the Dataflow Graph</summary>
> 
> The analysis required _7.52 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {9, 6}.
> We encountered no unknown side effects during the analysis.
> 
> ```r
...
> <summary style="color:gray">R Code of the Dataflow Graph</summary>
> 
> The analysis required _5.14 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {10, 1, 3}.
> We encountered no unknown side effects during the analysis.
> 
> ```r
...
> ```
> 	
> (The analysis required _5.08 ms_ (including parsing with the R&nbsp;shell) within the generation environment.)
> 
> 
> 	
> 
...
The analysis required _3.35 ms_ (including parse and normalize) within the generation environment. 
We encountered no unknown side effects during the analysis.
...
The analysis required _4.67 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {4->0}.
We encountered unknown side effects (with ids: 6 (linked)) during the analysis.
...
> <summary style="color:gray">R Code of the Dataflow Graph</summary>
> 
> The analysis required _3.92 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {9->7, 7->3, 4->0}.
...
> <summary style="color:gray">R Code of the Dataflow Graph</summary>
> 
> The analysis required _5.22 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {3->2}.
> We encountered no unknown side effects during the analysis.
> 
> ```r
> for(i in v) x <- x + 1
...
The analysis required _2.25 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {7->0}.
We encountered no unknown side effects during the analysis.
...
The analysis required _4.64 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {4->1}.
We encountered no unknown side effects during the analysis.
...
The analysis required _1.81 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {0->1, 0->2}.
We encountered no unknown side effects during the analysis.
...
The analysis required _2.92 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {0}.
We encountered no unknown side effects during the analysis.
...
The analysis required _6.79 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {0->4, 0->3, 1->3}.
We encountered no unknown side effects during the analysis.
...
The analysis required _6.41 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {0->3}.
We encountered no unknown side effects during the analysis.
...
The analysis required _5.73 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {7->4}.
We encountered no unknown side effects during the analysis.
...
The analysis required _4.79 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {6->1}.
We encountered no unknown side effects during the analysis.
...
The analysis required _4.67 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {11->1, 1->11}.
We encountered no unknown side effects during the analysis.
...
The analysis required _3.99 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {1, 1->5, 9->5}.
We encountered no unknown side effects during the analysis.
...
The analysis required _5.55 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {11->1, 1->11}.
We encountered no unknown side effects during the analysis.
...
The analysis required _3.74 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {5->1, 5->3}.
We encountered no unknown side effects during the analysis.
...
The analysis required _5.34 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {3->10}.
We encountered no unknown side effects during the analysis.
...
The analysis required _4.06 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {3->1}.
We encountered no unknown side effects during the analysis.
...
> <summary style="color:gray">R Code of the Dataflow Graph</summary>
> 
> The analysis required _2.52 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {2, 4->2}.
> We encountered no unknown side effects during the analysis.
> 
> ```r
> for(i in v) b
...
> <summary style="color:gray">R Code of the Dataflow Graph</summary>
> 
> The analysis required _4.42 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {1, 3->1}.
> We encountered no unknown side effects during the analysis.
> 
> ```r
> while(TRUE) b
...
The analysis required _3.82 ms_ (including parse and normalize) within the generation environment. The following marks are used in the graph to highlight sub-parts (uses ids): {5->3, 5->1, 5->2}.
We encountered no unknown side effects during the analysis.
...
The analysis required _6.86 ms_ (including parse and normalize) within the generation environment. 
We encountered no unknown side effects during the analysis.
...
The analysis required _5.42 ms_ (including parse and normalize) within the generation environment. 
We encountered no unknown side effects during the analysis.
...
The analysis required _4.20 ms_ (including parse and normalize) within the generation environment. 
We encountered no unknown side effects during the analysis.
...
The analysis required _9.75 ms_ (including parse and normalize) within the generation environment. 
We encountered no unknown side effects during the analysis.
...
The analysis required _7.60 ms_ (including parse and normalize) within the generation environment. 
...
The analysis required _5.66 ms_ (including parse and normalize) within the generation environment. 
