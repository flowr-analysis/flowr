_This document was generated automatically from '/home/limerent/GitHub/phd/flowr/src/documentation/print-interface-wiki.ts' on 2024-09-28, 12:55:18 UTC presenting an overview of flowR's interfaces (version: 2.0.25, using R version 4.4.1)._
...
The complete round-trip took 12.33 ms (including time required to validate the messages, start, and stop the internal mock server).
...
```json
{
  ...
  "type": "error",
  "fatal": false,
  "reason": "Error while analyzing file sample.R: GuardError: unable to parse R code (see the log for more information) for request {\"request\":\"file\",\"content\":\"/tmp/tmp-107164-pE2fxJX3Awry-.R\"}}"
}
```
...
The complete round-trip took 1.11 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The complete round-trip took 8.16 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The complete round-trip took 5.87 ms (including time required to validate the messages, start, and stop the internal mock server).
...
> [!WARNING]
> To execute arbitrary R commands with a request, the server has to be started explicitly with <span title="Description: Allow to access the underlying R session when using flowR (security warning: this allows the execution of arbitrary R code!)">`--r-session-access`</span>.
> Please be aware that this introduces a security risk.


The REPL execution message allows to send a REPL command to receive its output. 
For more on the REPL, see the [introduction](https://github.com/flowr-analysis/flowr/wiki//Overview#the-read-eval-print-loop-repl), or the [description below TODO TODO](#using-the-repl).
...
> Please be very careful (and do not use <span title="Description: Allow to access the underlying R session when using flowR (security warning: this allows the execution of arbitrary R code!)">`--r-session-access`</span> if you are unsure).
...
<summary> (1) Request: <b><code>request-repl-execution</code> Message</b></summary>
...
<summary> (2) Response: <code>response-repl-execution</code> Message</summary>
...
<summary> (3) Response: <code>end-repl-execution</code> Message</summary>
...
The complete round-trip took 0.99 ms (including time required to validate the messages, start, and stop the internal mock server).
