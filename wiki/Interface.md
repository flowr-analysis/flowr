_This document was generated from 'src/documentation/print-interface-wiki.ts' on 2024-11-07, 10:12:24 UTC presenting an overview of flowR's interfaces (v2.1.5, using R v4.4.0)._
...
The complete round-trip took 16.29 ms (including time required to validate the messages, start, and stop the internal mock server).
...
```json
{
  ...
  "type": "error",
  "fatal": false,
  "reason": "Error while analyzing file sample.R: GuardError: unable to parse R code (see the log for more information) for request {\"request\":\"file\",\"content\":\"/tmp/tmp-5451-mFsIzJEjB7Ze-.R\"}}"
}
```
...
The complete round-trip took 6.70 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The complete round-trip took 1.17 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The complete round-trip took 20.48 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The complete round-trip took 3.92 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The analysis required _1.87 ms_ (incl. parse and normalize) within the generation environment. 
