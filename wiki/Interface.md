_This document was generated from 'src/documentation/print-interface-wiki.ts' on 2024-10-22, 17:01:29 UTC presenting an overview of flowR's interfaces (v2.1.3, using R v4.4.1)._
...
The complete round-trip took 13.85 ms (including time required to validate the messages, start, and stop the internal mock server).
...
```json
{
  ...
  "type": "error",
  "fatal": false,
  "reason": "Error while analyzing file sample.R: GuardError: unable to parse R code (see the log for more information) for request {\"request\":\"file\",\"content\":\"/tmp/tmp-28129-Ahx1PQjx34t9-.R\"}}"
}
```
...
The complete round-trip took 6.37 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The complete round-trip took 6.30 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The complete round-trip took 20.02 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The complete round-trip took 3.66 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The `RShell` class interfaces with the `R`&nbsp;ecosystem installed on the host system.
