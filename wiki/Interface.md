_This document was generated from 'src/documentation/print-interface-wiki.ts' on 2024-10-04, 17:57:14 UTC presenting an overview of flowR's interfaces (v2.0.25, using R v4.4.0)._
...
The complete round-trip took 16.79 ms (including time required to validate the messages, start, and stop the internal mock server).
...
```json
{
  ...
  "type": "error",
  "fatal": false,
  "reason": "Error while analyzing file sample.R: GuardError: unable to parse R code (see the log for more information) for request {\"request\":\"file\",\"content\":\"/tmp/tmp-5505-r9MEjIfd7F8Q-.R\"}}"
}
```
...
The complete round-trip took 1.95 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The complete round-trip took 10.73 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The complete round-trip took 7.62 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The complete round-trip took 7.53 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The complete round-trip took 21.02 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The complete round-trip took 4.41 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The analysis required _3.64 ms_ (including parsing and normalization) within the generation environment. 
