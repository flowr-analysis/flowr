_This document was generated from 'src/documentation/print-interface-wiki.ts' on 2024-10-15, 18:40:12 UTC presenting an overview of flowR's interfaces (v2.1.3, using R v4.4.1)._
...
The complete round-trip took 18.05 ms (including time required to validate the messages, start, and stop the internal mock server).
...
```json
{
  ...
  "type": "error",
  "fatal": false,
  "reason": "Error while analyzing file sample.R: GuardError: unable to parse R code (see the log for more information) for request {\"request\":\"file\",\"content\":\"/tmp/tmp-361236-RQNKl43KCMc3-.R\"}}"
}
```
...
The complete round-trip took 10.74 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The complete round-trip took 7.17 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The complete round-trip took 17.68 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The complete round-trip took 3.65 ms (including time required to validate the messages, start, and stop the internal mock server).
...
The analysis required _1.69 ms_ (incl. parse and normalize) within the generation environment. 
