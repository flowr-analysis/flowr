_This document was generated from '[src/documentation/wiki-package-database.ts](https://github.com/flowr-analysis/flowr/tree/main//src/documentation/wiki-package-database.ts)' on 2026-07-13, 19:48:03 UTC presenting an overview of flowR's bundled CRAN package database that resolves `library()` calls (v2.11.1, using R v4.6.0). Please do not edit this file/wiki page directly._
# Package Database
...
| `base-full` | base-R packages (`base`, `stats`, `graphics`, ...) | full history | 23 | 1,626 | 418 KB | ≈ 19 ms |
| `current-top` | the 1,000 most-downloaded CRAN packages | latest only | 1,000 | 1,000 | 1.9 MB | ≈ 65 ms |
| `current-rest` | the remaining CRAN packages | latest only | 22,742 | 22,742 | 14.0 MB | ≈ 360 ms |
| `shared dictionary` | 1,394,438 strings shared by every shard | - | - | - | 6.2 MB | ≈ 96 ms |
...
analysis time). Measured here at generation time: opening the bundle took 44 ms, and a warmed per-package export lookup takes 130 µs. Each `library()` or `::` in a script is then one cached lookup.