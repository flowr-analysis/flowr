_This document was generated from '[src/documentation/wiki-package-database.ts](https://github.com/flowr-analysis/flowr/tree/main//src/documentation/wiki-package-database.ts)' on 2026-07-14, 09:05:36 UTC presenting an overview of flowR's bundled CRAN package database that resolves `library()` calls (v2.11.1, using R v4.6.0). Please do not edit this file/wiki page directly._
# Package Database
...
<i>Defined at <a href="https://github.com/flowr-analysis/flowr/tree/main/src/documentation/wiki-package-database.ts#L16">src/documentation/wiki-package-database.ts#L16</a></i>
...
You can also analyze a package straight off disk and <a href="https://github.com/flowr-analysis/flowr/tree/main/src/project/plugins/package-version-plugins/flowr-analyzer-package-versions-sigdb-plugin.ts#L109"><code><span title="Analyze a single installed R package or a whole library folder on disk -- running flowR over its R/ sources to extract each function's full signature (parameters, defaults, callees) alongside its DESCRIPTION/NAMESPACE metadata -- and add it as a resolvable source. Returns the packages added.">addLocalPackages</span></code></a>
...
<a href="https://github.com/flowr-analysis/flowr/tree/main/src/project/sigdb/reader.ts#L382"><code><span title="A transparent, read-only view over several SigDatabase shards described by a SigDbManifest . When the manifest embeds each shard's index (the default), openManifest() reads only that small file to build the package to shard routing table -- **no shard is decompressed up front**. A shard's .br is decompressed into the hash-keyed cache and opened only the first time a package it holds is queried (or...">SigDatabaseSet</span></code></a>). Nothing is read when the manifest opens. The first lookup of a package
...
| `base-full` | base-R packages (`base`, `stats`, `graphics`, ...) | full history | 23 | 1,626 | 418 KB | ≈ 23 ms |
| `current-top` | the 1,000 most-downloaded CRAN packages | latest only | 1,000 | 1,000 | 1.9 MB | ≈ 54 ms |
| `current-rest` | the remaining CRAN packages | latest only | 22,742 | 22,742 | 14.0 MB | ≈ 480 ms |
| `shared dictionary` | 1,394,438 strings shared by every shard | - | - | - | 6.2 MB | ≈ 170 ms |
...
analysis time). Measured here at generation time: opening the bundle took 84 ms, and a warmed per-package export lookup takes 140 µs. Each `library()` or `::` in a script is then one cached lookup.