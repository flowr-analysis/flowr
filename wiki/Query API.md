_This document was generated from '[src/documentation/print-query-wiki.ts](https://github.com/flowr-analysis/flowr/tree/main//src/documentation/print-query-wiki.ts)' on 2025-01-16, 18:06:08 UTC presenting an overview of flowR's query API (v2.2.0, using R v4.4.0). Please do not edit this file/wiki page directly._
...
(The analysis required _25.76 ms_ (including parse and normalize) within the generation environment.)
...
The analysis required _11.62 ms_ (including parsing and normalization and the query) within the generation environment.	
...
The analysis required _2.45 ms_ (including parsing and normalization and the query) within the generation environment.	
...
The analysis required _7.15 ms_ (including parsing and normalization and the query) within the generation environment.	
...
The analysis required _1.71 ms_ (including parse and normalize) within the generation environment. 
...
The analysis required _2.90 ms_ (including parsing and normalization and the query) within the generation environment.	
...
Using the example code `x <- 1;y <- 2`, the following query returns that the first assignment happens always before the other:
...
The analysis required _1.55 ms_ (including parse and normalize) within the generation environment. 
We encountered no unknown side effects during the analysis.
...
The analysis required _1.24 ms_ (including parse and normalize) within the generation environment. 
We encountered no unknown side effects during the analysis.
...
The analysis required _1.26 ms_ (including parse and normalize) within the generation environment. 
We encountered no unknown side effects during the analysis.
...
The analysis required _2.53 ms_ (including parsing and normalization and the query) within the generation environment.	
...
The analysis required _6.00 ms_ (including parsing and normalization and the query) within the generation environment.	
