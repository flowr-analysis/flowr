_This document was generated from '[src/documentation/wiki-query.ts](https://github.com/flowr-analysis/flowr/tree/main//src/documentation/wiki-query.ts)' on 2026-07-14, 04:48:31 UTC presenting an overview of flowR's query API (v2.11.1, using R v4.6.0). Please do not edit this file/wiki page directly._
...
(The analysis required _27.7 ms_ (including parse and normalize, using the [r-shell](https://github.com/flowr-analysis/flowr/wiki/engines) engine) within the generation environment.)
...
_All queries together required ≈325 ms (1ms accuracy, total 326 ms)_

<details> <summary style="color:gray">Show Detailed Results as Json</summary>

The analysis required _326.1 ms_ (including parsing and normalization and the query) within the generation environment.
...
The analysis required _45.1 ms_ (including parsing and normalization and the query) within the generation environment.
...
_All queries together required ≈39 ms (1ms accuracy, total 40 ms)_

<details> <summary style="color:gray">Show Detailed Results as Json</summary>

The analysis required _39.5 ms_ (including parsing and normalization and the query) within the generation environment.
...
_All queries together required ≈27 ms (1ms accuracy, total 28 ms)_

<details> <summary style="color:gray">Show Detailed Results as Json</summary>

The analysis required _27.6 ms_ (including parsing and normalization and the query) within the generation environment.
...
The analysis required _14.7 ms_ (including parsing and normalization and the query) within the generation environment.
...
The analysis required _0.5 ms_ (including parse and normalize, using the [r-shell](https://github.com/flowr-analysis/flowr/wiki/Engines) engine) within the generation environment. 
We encountered no unknown side effects during the analysis.
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;- Exception maybe thrown at id **11** "stop" (2.13-35, cds: true:2.3-3.14)\
_All queries together required ≈7 ms (1ms accuracy, total 6 ms)_
...
The analysis required _5.9 ms_ (including parse and normalize, using the [r-shell](https://github.com/flowr-analysis/flowr/wiki/Engines) engine) within the generation environment. 
We encountered no unknown side effects during the analysis.
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;╰ _Metadata_: <code>totalFunctionCalls: 1, totalVariableUses: 0, suppressed: 0, searchTimeMs: 0, processTimeMs: 67</code>\
_All queries together required ≈76 ms (1ms accuracy, total 76 ms)_

<details> <summary style="color:gray">Show Detailed Results as Json</summary>

The analysis required _75.6 ms_ (including parsing and normalization and the query) within the generation environment.
...
```shell
:query @static-slice (3@print)i "source(\"library.R\"); print(1)"
```
...
&nbsp;&nbsp;&nbsp;╰ **visualize** (4 hits):\
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;╰ **text** (4 hits): _`mean(data$x)`_ (L.9), _`print(m)`_ (L.10), _`mean(data2$k)`_ (L.19), _`print(mean(data2$k))`_ (L.19)\
_All queries together required ≈17 ms (1ms accuracy, total 17 ms)_

<details> <summary style="color:gray">Show Detailed Results as Json</summary>

The analysis required _16.8 ms_ (including parsing and normalization and the query) within the generation environment.
