_This document was generated from '[src/documentation/wiki-query.ts](https://github.com/flowr-analysis/flowr/tree/main//src/documentation/wiki-query.ts)' on 2026-04-03, 10:01:28 UTC presenting an overview of flowR's query API (v2.10.2, using R v4.5.3). Please do not edit this file/wiki page directly._
...
flowR repl using flowR v2.10.2, R v4.5.3 (r-shell engine)
...
(This can be shortened to `@input-sources (3@print) "f <- function(x) {\n	x <- x * 2\n	print(x)\n}"` when used with the REPL command <span title="Description (Repl Command): Query the given R code, start with 'file://' to indicate a file. The query is to be a valid query in json format (use 'help' to get more information).">`:query`</span>).
