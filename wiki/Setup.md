_This document was generated from '[src/documentation/wiki-setup.ts](https://github.com/flowr-analysis/flowr/tree/main//src/documentation/wiki-setup.ts)' on 2026-07-13, 07:04:00 UTC presenting an overview of flowR's setup instructions (v2.11.1, using R v4.6.0). Please do not edit this file/wiki page directly._
...
tool, which analyses the whole of CRAN with _flowR_ itself and serialises the result into the bundle (see the
...
- you can inspect a loaded database from the REPL with the <span title="Description (Repl Command): Query the given R code (use 'help' for more information)">`:query`</span> command, e.g. `:query @sigdb <package> [<function>]`.
