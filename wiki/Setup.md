_This document was generated from '[src/documentation/wiki-setup.ts](https://github.com/flowr-analysis/flowr/tree/main//src/documentation/wiki-setup.ts)' on 2026-07-13, 19:16:33 UTC presenting an overview of flowR's setup instructions (v2.11.1, using R v4.6.0). Please do not edit this file/wiki page directly._
...
[Package Database](https://github.com/flowr-analysis/flowr/wiki/package-database) page for the format and how to point _flowR_ at your own database).
...
- only the tiny `base.*` **floor** (self-contained base-R signatures) is committed under `src/data/sigdb/`, so a fresh
  ...
  demand from the release with `:signature download` (see the [Package Database](https://github.com/flowr-analysis/flowr/wiki/package-database) page).
...
- you can inspect a loaded database from the REPL with the <span title="Description (Repl Command): Query the given R code (use 'help' for more information)">`:query`</span> command, e.g. `:query @signature <package> [<function>]`.
