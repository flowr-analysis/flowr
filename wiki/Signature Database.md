...
reads as "unknown" rather than "no"; <a href="https://github.com/flowr-analysis/flowr/tree/main/src/dataflow/environments/built-in-props.ts#L569"><code><span title="The part of a BuiltInFnInfo the signature database already knows: the parameter names in order with the ArgProp bits stored for each, plus the SigDbProps properties; everything else is dropped.">fnInfoFromSignature</span></code></a> hands the mask on unchanged, which is
...
|-------|----------|---------------|---------:|---------:|-------------:|-------------------:|
| `base-current` | base-R packages (`base`, `stats`, `graphics`, ...) | latest only | 23 | 23 | 104 KB | ≈ 360 µs |
| `base-full` | base-R packages (`base`, `stats`, `graphics`, ...) | full history | 23 | 1,626 | 468 KB | ≈ 2.5 ms |
| `current-top` | the 1,000 most-downloaded CRAN packages | latest only | 1,000 | 1,000 | 2.1 MB | ≈ 6.2 ms |
| `current-rest` | the remaining CRAN packages | latest only | 22,742 | 22,742 | 15.1 MB | ≈ 100 ms |
| `history-rest` | the remaining CRAN packages | full history | 18,466 | 140,128 | 30.7 MB | ≈ 200 ms |
