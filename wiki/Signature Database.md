_This document was generated from '[src/documentation/wiki-signature-database.ts](https://github.com/flowr-analysis/flowr/tree/main/src/documentation/wiki-signature-database.ts)' on 2026-08-09, 16:08:24 UTC presenting an overview of flowR's bundled signature database that resolves `library()` calls (v2.13.10, using R v4.6.1). Please do not edit this file/wiki page directly._
# Signature Database
...
- the transitive call graph <a href="https://github.com/flowr-analysis/flowr/tree/main/src/project/sigdb/reader.ts#L611"><code>SigDatabase::<b>transitiveCallees</b></code></a>, expanding the stored local callees inside one version
...
<a href="https://github.com/flowr-analysis/flowr/tree/main/src/project/sigdb/reader.ts#L723"><code><span title="A transparent, read-only view over several SigDatabase shards described by a SigDbManifest . When the manifest embeds each shard's index (the default), openManifest() reads only that small file to build the package to shard routing table.">SigDatabaseSet</span></code></a>). Nothing is read when the manifest opens. The first lookup of a package
...
| `base-full` | base-R packages (`base`, `stats`, `graphics`, ...) | full history | 23 | 1,626 | 468 KB | ≈ 6.1 ms |
| `current-top` | the 1,000 most-downloaded CRAN packages | latest only | 1,000 | 1,000 | 2.1 MB | ≈ 23 ms |
| `current-rest` | the remaining CRAN packages | latest only | 22,742 | 22,742 | 15.1 MB | ≈ 400 ms |
| `history-rest` | the remaining CRAN packages | full history | 18,466 | 140,128 | 30.7 MB | ≈ 410 ms |
...
(<a href="https://github.com/flowr-analysis/flowr/tree/main/src/project/sigdb/reader.ts#L723"><code><span title="A transparent, read-only view over several SigDatabase shards described by a SigDbManifest . When the manifest embeds each shard's index (the default), openManifest() reads only that small file to build the package to shard routing table.">SigDatabaseSet</span></code></a>), and which information gets stored is selectable (<a href="https://github.com/flowr-analysis/flowr/tree/main/src/project/sigdb/schema.ts#L237"><code><span title="Which information to store in a bundle (default: everything). Turning a feature off shrinks the database -- e.g. an exports-and-dependencies-only bundle omits the (largest) call graphs and signatures. The export view (exported/internal/deprecated) is always available.">SigDbFeatures</span></code></a>).
