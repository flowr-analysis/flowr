...
reads as "unknown" rather than "no"; <a href="https://github.com/flowr-analysis/flowr/tree/main/src/dataflow/environments/built-in-props.ts#L572"><code><span title="The part of a BuiltInFnInfo the signature database already knows: the parameter names in order with the ArgProp bits stored for each, plus the SigDbProps properties; everything else is dropped.">fnInfoFromSignature</span></code></a> hands the mask on unchanged, which is
...
<a href="https://github.com/flowr-analysis/flowr/tree/main/src/project/sigdb/signature-db.ts#L67"><code><span title="Every loaded source as one, for the questions this interface does not ask.">SignatureDb::<b>sources</b></span></code></a> is the escape hatch to the raw sources for what the interface above does not
...
|-------|----------|---------------|---------:|---------:|-------------:|-------------------:|
| `base-current` | base-R packages (`base`, `stats`, `graphics`, ...) | latest only | 23 | 23 | 104 KB | ≈ 710 µs |
| `base-full` | base-R packages (`base`, `stats`, `graphics`, ...) | full history | 23 | 1,626 | 468 KB | ≈ 3.6 ms |
| `current-top` | the 1,000 most-downloaded CRAN packages | latest only | 1,000 | 1,000 | 2.1 MB | ≈ 10 ms |
