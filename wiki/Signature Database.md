...
analysis time). Opening the bundle is a one-off cost paid once per analysis, and a warmed per-package export lookup is a matter of microseconds. Each `library()` or `::` in a script is then one cached lookup.